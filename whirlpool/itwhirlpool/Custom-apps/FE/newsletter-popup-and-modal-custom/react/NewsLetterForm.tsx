...
const createOrUpdateUser = (
  ...
  return fetch(fetchUrlPatch, options);
};
...
const NewsLetterForm: StorefrontFunctionComponent<NewsLetterFormProps> = ({
  ...
  function checkBoxValidation() {
    const el = document.querySelector(
      ".itwhirlpool-newsletter-popup-custom-0-x-informativa"
    ) as HTMLInputElement;
    ...
    if (!emailValue || !nameValue || !surnameValue || !privacy) {
      ...
      getIdUser(emailValue).then((User: any) => {
        if (User.length > 0 && !User[0].isNewsletterOptIn) {
          fetch("/api/sessions?items=*")
            ...
            .then((res: any) => {
              if (
                ...
                createOrUpdateUser(emailValue, campaignToSend, nameValue, surnameValue);
              }
              setLoading(false);
              setSuccess(true);
            });
          ...
          createOrUpdateUser(emailValue, campaignToSend, nameValue, surnameValue).then(
            (repsonse: any) => {
