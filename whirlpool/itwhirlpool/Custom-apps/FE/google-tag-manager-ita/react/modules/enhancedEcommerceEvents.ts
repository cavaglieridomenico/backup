...
export async function sendEnhancedEcommerceEvents(e: PixelMessage) {
  ...
  switch (e.data.eventName) {
    ...
    case 'vtex:productView': {
      ...
      push({
        ...
        contentIndex: document.getElementsByClassName(
          'itwhirlpool-video-player-thron-pdp-0-x-figure itwhirlpool-video-player-thron-pdp-0-x-figure--productPage'
        ).length,
      })
    ...
    //   const filtri = window.dataLayer?.filter(
    ...
    //     if (
    //       filtri[filtri.length - 1]?.filterChainedDetails.includes(
    ...
    //         filtri[filtri.length - 1]?.filterChainedDetails
    //       }&filters ${e.data.items.filterName.replace(':', '').toLowerCase()}=${
    ...
    //   filtri.map((filtro) => {
    //     if (
    //       e.data.items.filterValue == filtro.filterValue &&
    //       filtro.filterInteraction === 'select'
    //     ) {
    //       filterInteraction = 'remove'
    //     }
    //     if (
    //       e.data.items.filterValue == filtro.filterValue &&
    //       filtro.filterInteraction === 'remove'
    //     ) {
    ...
    case 'vtex:prodDetailsTab': {
      ...
      if (tabName === 'caratteristiche--principali') {
        push({
          event: 'prodDetailsTab',
    ...
    case 'vtex:wellBeing_discoverMore': {
      ...
      const linkClassLabel =
        'itwhirlpool-store-link-custom-0-x-link--secondColLink'
    ...
    case 'vtex:cta_clickBanner100years': {

      const objEvent = {
        /* GA4 */
        event: 'cta_click',
        link_url: e.data.linkUrl,
        link_text: 'Scopri i vantaggi',
        checkpoint: e.data.checkpoint,
        /*  */
        /* UA */
        eventCategory: 'CTA Click',
        eventAction: 'Scopri i vantaggi',
        eventLabel: 'www.whirlpool.it/servizi-e-vantaggi',
        /*  */
        /* GA4 */
        area: e.data.clickArea,
        type: e.data.type,
        /*  */
      }
      push(objEvent)
