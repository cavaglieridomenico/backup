...
const PromotionsSubmenu: StorefrontFunctionComponent<PromotionsSubmenuProps>= ({ items }) => {
  ...
  return (
      <div className={isMobile ? style.containerMobile : style.containerDesktop}>
        {containerPadding(items.map((item:ItemProps) =>{
          return <Item itemTitle={item.itemTitle} itemIcon={item.itemIcon} itemLink={item.itemLink} itemSubtitle={item.itemSubtitle} width={getWidth(isMobile)} />
        }))}
      </div>
    )
}
