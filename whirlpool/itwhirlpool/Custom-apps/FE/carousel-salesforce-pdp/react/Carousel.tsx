...
const Carousel = (props: Props) => {
  ...
  const ArrowContainerRender = ({ children }: ArrowContainerProps) => {
    ...
    const containerClasses = classnames(
      ...
      's h-100 mw9 flex-ns justify-between items-center dn-s'
    )
  ...
  const banners: BannerProps[] = bannersProp.filter(
    banner => banner && (banner.mobileImage || banner.image_link)
  )
  ...
  return (
    <SliderContainer
      ...
      autoplayInterval={autoplayInterval * 100000}
      pauseOnHover
      onNextSlide={handleNextSlide}
      className={classnames(styles.container, '  mw9 items-center justify-center')}
    >
      <Slider
        loop
        easing="ease"
        className={classnames(styles.sliderFrame, 'vtex-slider-0-x-sliderFrame list pa0 h-100 ma0 flex justify-center')}
        perPage={PER_PAGE}
      ...
      banners: {
        items: {
          properties: {
            image: {
              default: '',
              title: 'admin/editor.carousel.banner.image_link.title',
              type: 'string',
              widget: {
