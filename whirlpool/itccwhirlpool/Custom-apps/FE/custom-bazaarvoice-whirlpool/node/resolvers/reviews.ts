...
const DEFAULT_REVIEWS_INCLUDE = "Products,Comments"
const DEFAULT_REVIEWS_STATS = "Reviews"
const DEFAULT_REVIEWS_SEARCH = null
const DEFAULT_REVIEWS_SORT = null
...
const getSyndacate = (syndacate: any) => {
  const alternativeNull = {
    ClientId: null,
    ContentLink: null,
    LogoImageUrl: null,
    Name: null,
  }
  return syndacate !== undefined ? syndacate : alternativeNull
}
...
export const queries = {
  productReviews: async (
    ...
    const { sort, offset, pageId, filter, quantity, include, stats, search } = args
    const {
    ...
    return apps.getAppSettings(appId).then(async (appSettings: any) => {
      ...
      const newSort: string | null = sort || DEFAULT_REVIEWS_SORT
      const newInclude: string | null = include || DEFAULT_REVIEWS_INCLUDE
      const newStats: string | null = stats || DEFAULT_REVIEWS_STATS
      const newSearch: string | null = search || DEFAULT_REVIEWS_SEARCH
          ...
          const productExtended: ProductGraphQL = {
            ...
            ReviewStatistics: {
              ...
              RatingDistribution: [1, 2, 3, 4, 5].map((i) => {
                const currentRating =
                  currentProduct?.ReviewStatistics?.RatingDistribution?.find(
      ...
      return {
        ...
        Results: reviews.Results.map((result) => {
          ...
          return {
            ...
            SyndicationSource: getSyndacate(result.SyndicationSource),
          }
        }),
      }
    })
