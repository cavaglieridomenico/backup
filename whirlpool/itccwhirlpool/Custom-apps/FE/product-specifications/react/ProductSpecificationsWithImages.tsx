				...
				if (fieldsIds && fieldsIds.length) {
					...
					Promise.all(
						...
						fields.forEach((field: FieldDataModel) => {
							...
							const idBrand = Object.keys(description).find(el => el === brand);
							const descriptionValue = idBrand ? description[idBrand] : "";
							srcUrls[field.Name] = descriptionValue;
						})
							...
							(targetGrp.specifications as Array<Specification>).forEach(
								(spec: Specification) => {
									tempEnrichedSpecs.push({
										...spec,
										src: xss(srcUrls[spec.originalName]),
									})
								},
							)
					...
					<div className={styles.right}>
						...
						{spec &&
							spec.values.map((val, valueIndex) => (
								...
								>
									<ReadMore text={val} data={spec} context={product} matchId={spec.name.toLowerCase().replace(/\s/g, '-')}></ReadMore>
