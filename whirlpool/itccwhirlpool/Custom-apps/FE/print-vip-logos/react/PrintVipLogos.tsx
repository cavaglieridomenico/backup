...
const PrintVipLogos: StorefrontFunctionComponent<PrintVipLogosProps> = ({
  children,
}) => {
  /* component State */
  ...
  return (
    <>
      {state.isVip && data.GetVipLogo != null ? (
        <div className={style.containerImage}>
          {data && <img className={style.vipImage} src={data.GetVipLogo} />}
        </div>
      ) : (
        <>{children}</>
      )}
