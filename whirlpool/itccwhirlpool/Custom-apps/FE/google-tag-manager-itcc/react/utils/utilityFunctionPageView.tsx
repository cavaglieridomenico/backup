...
export async function getIdCategory(categoryPath: string) {
  ...
  const isDishwashing = window.location.pathname.includes('lave-vaisselle')
  ...
  let category = cleanCategories.filter(
    (cat: any) =>
      normalizeString(cat.name).replaceAll(' ', '-') ==
      normalizeString(categoryPath)
  )[0]
  const otherProductCategoryId = 8 // Can be found in: https://frccwhirlpool.myvtex.com/admin/Site/Categories.aspx
