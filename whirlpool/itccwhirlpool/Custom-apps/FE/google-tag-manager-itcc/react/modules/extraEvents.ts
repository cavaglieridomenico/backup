...
//@ts-ignore
async function getProductCategory(pageType: string, categoryPath: string, url:string) {
...
function getContentGroupingSecond(category): string {
  ...
  const mappedCategoriesFromUrl = {
    ...
    "climatizzazione": "Air conditioner",
    ...
    "lavaggio-e-asciugatura": "Laundry",
    "lavastoviglie": "Dishwash",
    "accessori": "Wpro Accessories",
  }
  ...
  if(isAccesories || category.toLowerCase().includes("accessories")){
    ...
    const contentGroupingFromProductCategory = mappedCategoriesFromProductCategory[catSplit[catSplit.length - 1]]
  ...
  // For PLP based on URL - fallback in case there is no product-category
      ...
      const contentGroupingFromUrl = mappedCategoriesFromUrl[splittedUrl?.[splittedUrl?.length - 2]]
...
function getContentGrouping(url: string) {
  ...
  const previousPageViewEvents = window.dataLayer.filter(item => item?.event == "pageView")
  const previousContentGrouping: string = previousPageViewEvents ? previousPageViewEvents[(previousPageViewEvents.length) - 1]?.contentGrouping : ""
  const previousPageType: string = previousPageViewEvents ? previousPageViewEvents[(previousPageViewEvents.length) - 1]?.pageType : ""

  if (searchParams?.includes("_q") || previousPageType === "search") {
    return "(Other)"
  }
  else if (urlWithoutQueryStrings?.includes("produits") || urlWithoutQueryStrings?.includes("accessoires") || previousContentGrouping === "Catalog") {  //I'm in plp
    return "Catalog"
  }
...
export async function sendExtraEvents(e: PixelMessage) {
  switch (e.data.eventName) {
    ...
    //   let categoryPaths = e.data.pageUrl.replace(e.origin, '').substr(1,).split("?")[0].split('/')
    ...
    //   const productCategoryPath = decodeURI(categoryPaths[categoryPaths.length -1])
    ...
    //     }
    //     default : {
    //       productCategory = await getProductCategory(pageType, productCategoryPath, urlShort);
    //     }
