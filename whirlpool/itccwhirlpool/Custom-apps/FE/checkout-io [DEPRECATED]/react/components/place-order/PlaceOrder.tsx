...
const PlaceOrder: React.FC = () => {
  ...
  const { appSettings } = useAppSettings();
	...
	const isWPPayment =
		paymentData?.payments?.some((payment) => payment.paymentSystem == appSettings.worldpayConnectorId) ||
		false
	...
	const [orderMutation]: any = useMutation(placeOrder, {
		onCompleted: (data) => {
			...
			} else if (paymentData?.payments[0]?.paymentSystem == appSettings.worldpayConnectorId) {
  ...
  const hdx_reserve_slot = (orderFormId: any) => {
