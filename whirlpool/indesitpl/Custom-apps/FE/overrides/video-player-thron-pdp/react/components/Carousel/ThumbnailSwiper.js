...
const Thumbnail = props => {
	const { alt, isVideo, thumbUrl, handles, url = "", aspectRatio = 'auto' } = props
...
const ThumbnailSwiper = props => {
	...
	const productContexInfo = useProduct()
	const {
		isThumbsVertical,
		slides,
		thumbUrls,
	...
	return (
		<div className={thumbClassName} data-testid="thumbnail-swiper">
			<Swiper
				 ...
				 * one thumbnail */
				/* slidesPerGroup={displayThumbnailsArrows ? 3 : 1} */
				freeMode={false}
				mousewheel={false}
				zoom={false}
				...
				{slides.map((slide, i) => {
					return (
						<SwiperSlide
							...
							onClick={()=> {
								let normalizeName= productContexInfo.selectedItem.nameComplete.split(" ")

								let imageName= normalizeName.includes(productContexInfo.selectedItem.name)?
									normalizeName.filter(el=>el!==productContexInfo.selectedItem.name).join(" "):productContexInfo.selectedItem.nameComplete

								//Duplicated analytics event
								// window.dataLayer.push({
								// 	event: 'productImageClick',
								// 	productImageAsset: slide.thumbUrl || thumbUrls[i],
								// 	productImageName : imageName,
								// 	productImageCode : productContexInfo.selectedItem.name,
								//  	productImageFile : slide.url?slide.url:"Video"
								// })
								
								//GA4FUNREQ37
								handleClickAnal({ isVideo: (slide.type==="video"? true : false), url: slide.url })
							}}
						>
							<Thumbnail
