...
export async function sendEnhancedEcommerceEvents(e: PixelMessage) {
  switch (e.data.eventName) {
    ...
    case "vtex:productImpression": {
      ...
      Promise.all(parsedImpressions).then((values: any) => {
        if (values && values.length > 0) {
          push({
            event: "eec.impressionView",
            ecommerce: {
              currencyCode: currency !== "EUR" ? "EUR" : currency,
              impressions: values,
            },
          });
        }
    ...
    //       filterChainedDetails: `filters  ${ e.data.items.filterName} = ${e.data.items.filterValue}`
    //     });
    //   });
    //   break;
    // }
...
function findVariantImpression(data: any) {
  const result = data.properties.filter((el: any) => el.name === "Kolor");
  const result2 = data.items[0]?.variations[0]?.values[0] ? data.items[0].variations[0].values[0] : ""
  return result.length > 0 ?
    result[0].values[0] == "Non"
      && result.length >= 1 ?
...
function getDimension(product: any, dimension: number) {
  if (dimension === 4) {
    ...
    const result = product.properties.filter(
      (obj: any) => obj.name == "typ konstrukcji"
    );
    if (result.length > 0) {
...
function getListProductClick(data: any, _: string) {
  let prevPageArr: string[] = document.referrer.split("/")
  let currentPage: string[] = window.location.href.split("/")
  var listName = "";

  if (data.list !== undefined && !(prevPageArr.includes("do-it-together"))) {
    listName = "catalog_page_impression_list"
  }
  if (prevPageArr.length && prevPageArr.includes("do-it-together")) {
    listName = "campaign_page_impression_list"
  }
  if (data.list !== undefined && currentPage.length && currentPage.includes(data.list)) {
    listName = "product_page_up_selling_impression_list";
  }
  if (prevPageArr.length && !(prevPageArr.includes("do-it-together")) && data.list == undefined) {
    listName = "home_page_impression_list";
  }
    ...
    product.categoryId !== undefined
      ? getStringCategoryFromId(product.categoryId).then(async (respone) => {
        return {
          ...
          id: product.sku.name,
          price: `${product.sku.seller!.commertialOffer.Price}` === "0" ? "9999" : `${product.sku.seller!.commertialOffer.Price}`,
          brand: product.brand,
          category: respone.AdWordsRemarketingCode,
          variant: findVariantImpression(product),
          list: getList(
            list,
            product.categories,
