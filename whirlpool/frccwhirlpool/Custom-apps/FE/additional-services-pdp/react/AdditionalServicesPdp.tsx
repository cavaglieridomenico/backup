...
const AdditionalServicesPdp: StorefrontFunctionComponent<serviceProps> = ({
  ...
  /*--- GRAPHQL QUERY ---*/
  const [getSearchData, { data, loading }] = useLazyQuery(getAdditionalServices, {
    onCompleted: () => {
      if(!skuId)
      console.error("Product Context goes wrong")
    }
  });
  ...
  useEffect(() => {
    if(data) {
      setAdditionalServices([Livraison, ...data?.additionalServices]);
        setOpenQuestion(setInitialState([Livraison, ...data?.additionalServices]))
      setwarranty5years(data?.additionalServices?.filter((service: any) => formatName(service.Name) == "5 ans de garantie sur votre appareil" ).length > 0)
    }
  }, [data])
  ...
  const formatName = (name: string) => {
    return name.includes(`${tradePolicy}_`) ? name.split(`${tradePolicy}_`)[1] : name
  }
        ...
        <div className={style.serviceTitle}>
          {showTitle &&
            <p className={style.servTitleStyle}>
              {intl.formatMessage({id: 'store/additional-services-pdp.serviceTitle'})}
            </p>
          }
        </div>
            ...
            <Collapsible
              ...
              header={
                <div className={style.servOptions}>
                  ...
                  {service.Price === 0 ? (
                    <span className={style.freeLabel}>{intl.formatMessage({id: 'store/additional-services-pdp.freeLabel'})}</span>
                  ) : (
                    <span
              ...
              isOpen={ openQuestion && openQuestion[service.ServiceTypeId]?.isOpen }
              caretColor="#505050"
            >
              <div
