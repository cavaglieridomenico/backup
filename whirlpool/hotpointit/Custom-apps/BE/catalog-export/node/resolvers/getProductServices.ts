...
export const GetAdditionalServices = async (_: any, {skuId, salesChannelId}: {skuId: string, salesChannelId: string}, ctx: Context) => {
  ...
  if(!ctx.state.appSettings.isMarketplace){
    services = (await ctx.clients.Vtex.GetSKU(skuId)).Services?.filter((f: { ServiceTypeId: number }) => ids?.includes(f.ServiceTypeId+""))
  }else{
    process.env.SELLERACC = JSON.stringify(ctx.state.appSettings.sellerAccount);
    let sellerSkuId = (await ctx.clients.Vtex.GetSKU(skuId)).SkuSellers?.find((s: {SellerId: string}) => s.SellerId==ctx.state.appSettings.sellerAccount?.name)?.SellerStockKeepingUnitId;
