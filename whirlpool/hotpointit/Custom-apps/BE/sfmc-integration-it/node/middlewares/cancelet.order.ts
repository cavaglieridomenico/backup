...
export async function canceledCreateOrder(
    ...
    if(ctx.query.id!=undefined && ctx.query.id!=null && ctx.query.id!=""){
      try{
        ...
        let res = await checkToken(ctx, JSON.parse(process.env.TEST+"").key1Canc, JSON.parse(process.env.TEST+"").key2Canc, 0);
        ...
        if(e.response==undefined){
          ...
          ctx.message = "Cancel "+ctx.query.id+" - app settings: "+e?.response?.data?.message;
          ctx.vtex.logger.error("Cancel "+ctx.query.id+" - app settings: "+e?.response?.data?.message);
        }
      }
    }else{
      ctx.status = 400;
