export async function GetOrder(ctx: Context, next: () => Promise<any>) {
  ...
  if (user.length > 0 && user[0].email == email) {
    ctx.status = 200
    let fiscaldata = order.customData?.customApps?.find((app: { id: string }) => app.id == "fiscaldata")?.fields
    let pec = fiscaldata?.SDIPEC
    ctx.body = {
      pec: pec && pec != "null" ? pec : "",
      ...order.invoiceData,
      invoiceName: order.clientProfileData.tradeName ? order.clientProfileData.tradeName : "",
      fiscalCode: order.clientProfileData.document ? order.clientProfileData.document : fiscaldata?.codiceFiscaleAzienda,
      phone: order.clientProfileData.phone ? order.clientProfileData.phone : "",
      requestInvoice: order.customData?.customApps[0].fields?.requestInvoice
    }
  } else {
    ctx.status = 403
  }
