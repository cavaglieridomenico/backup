    ...
    try{
      ...
      if(promo.collections.length>0 && !found){
        ...
        for(let i=0;i<results.length && !found;i++){
          if(results[i].Data.find(f => f.SkuId == skuId)){
            if(promo.itemMinPrice==0 && promo.itemMaxPrice==0){
              found = true;
            }else{
              if(price>=promo.itemMinPrice && price<=promo.itemMaxPrice){
...
export async function GetCouponBySkuId(ctx: Context, next: () => Promise<any>) {
  ...
  if(isValid(ctx.query.skuId) && !isNaN(ctx.query.skuId)){
    try{
      ...
      skuContInvUtmPromises.push(
        new Promise<any>((resolve,reject) => {
          ctx.clients.vtexAPI.getStockBySellerName(ctx.query.skuId, 1, seller)
            .then(res => {
              resolve(res);
            })
            .catch(err => {
        ...
        arrayPromises.push(
          new Promise<any>((resolve,reject) => {
            ...
            .then(res => {
              ...
              (f.utmSource!="" || f.utmCampain!="") && !utmCodesToExclude.find(u => u?.utmCode==f.utmSource) && !utmCodesToExclude.find(u => u?.utmCode==f.utmCampain)));
            })
            .catch(err => {
              reject(err);
            })
