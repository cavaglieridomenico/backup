...
const ServiceLocatorRegions:StorefrontFunctionComponent<ServiceLocatorRegionsProps> = () => {
  ...
  useEffect(() => {
    let regions:any ={}
    fetch(
      'https://api.d2c.service-locator.wpsandwatch.com/v1/geo/regions?brand=HP&country=IT&locale=it_IT&country_id=Italia',
      {
        headers: {
  ...
  return <>
  <Helmet>
    <title> Cerca un tecnico  | Hotpoint Italia</title>
  </Helmet>
    ...
    <ul> 
      {regions.map((region:Region) =>
        ...
        <b><Link
          ...
          params={{ region: region.label.replace(/ /g, '-').toLowerCase(), slug: region.label }}
        >
          {region.label}
        </Link></b>
        </li>
