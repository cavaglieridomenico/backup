...
export const queries = {
  productReviews: async (
    ...
    return apps.getAppSettings(appId).then(async (appSettings: any) => {
      reviewsClient.env = appSettings.environment == 'staging'? 'http://stg.api.bazaarvoice.com' : 'http://api.bazaarvoice.com'
  ...
  getReview: async (_: any, { reviewId, appKey }: any, ctx: Context) => {
    ...
    reviewsClient.env = settings.environment == 'staging'? 'http://stg.api.bazaarvoice.com' : 'http://api.bazaarvoice.com'
