...
const feRPlp = (
  ...
  const isCategory = () => {
    return searchContext.variables.map.split(",").filter((el: any) => el.indexOf("category") !== -1|| el === "c").length > 0;
  };
  ...
  if (
    ...
    let categories = getCategory(searchContext.variables.map,searchContext.variables.query) 
    fetch('/api/catalog_system/pub/category/tree/5',{method:"GET"}).then(res => res.json()).then(tree => {
...
const FeReady: StorefrontFunctionComponent<feReadyProps> = ({
  ...
  useEffect(() => {
    ...
    //thumbnails
    var thumbs = document.getElementsByClassName("hotpointit-video-player-thron-pdp-0-x-carouselThumbBorder--productPage");
    for (var i = 0; i < thumbs.length; i++) {
      thumbs[i].addEventListener('click', (e: any) => {
        ...
        dataLayer.push({
          ...
          'productImageFile': image && !image.classList.contains("hotpointit-video-player-thron-pdp-0-x-thumbImg--productPage--video") ? thronId.replace("https://whirlpool-cdn.thron.com/delivery/public/thumbnail/whirlpool/", "") : "Video"
        })
      }, false);
    }
  }, [])
