...
const Search: StorefrontFunctionComponent<SearchProps> = ({
 ...
 /*localCheckApi*/
  ...
  const buildUrl = (obj: any) => {
    let url = obj["modelNumber"] ? `/ersatzteile/bom/${normalizeModelNumber(obj["modelNumber"]).toLowerCase()}-${obj["industrialCode"].toLowerCase()}` : `/ersatzteile/bom/${obj["industrialCode"].toLowerCase()}`;
  ...
  return (
    <div className={styles.custom_search}>
      ...
      { !inputValue && iconEmpty && !showLoadingMask && (
        <img className={isPlp ? styles.custom_search_icon_spare_plp : styles.custom_search_icon} src={iconEmpty}/>
      )}

      {!showSearchIcon && showLoadingMask && (
        <img className={isPlp ? styles.custom_search_icon_spare_plp : styles.custom_search_icon} src={loadingIcon}/>

      )}  

      { !showSearchIcon && inputValue  && !showLoadingMask && (
        <img className={isPlp ? styles.custom_search_icon_spare_plp : styles.custom_search_icon} src={iconClear}onClick={() => {
          setInputValue("");
          setlocalCheckObj(undefined);
          setResults([])
        }}/>
