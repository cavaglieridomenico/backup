...
import { CustomProductsList } from "bauknechtde.custom-products-list";
...
const Bom: StorefrontFunctionComponent<BomProps> = ({
  ...
  homeLabel = "home",
  sparePartsLabel = "Ersatzteile",
  sparePartsModel = "Modell",
  ...
  const params =
    Object.keys(runtime.route.params).length > 0
      ? runtime.route.params.ids.split("-")
      : [
        runtime.route.queryString.modelNumber,
        runtime.route.queryString.industrialCode,
      ];
  ...
  useEffect(() => {
    setFilter(
      [
        ...
        activeFilterList.map((fl) => {
          return { name: "familyGroup", value: fl };
        })
      )
    );
  }, [activeFilterList]);
  ...
  const onFiltersDone = (data) => {
    setFilters(
      data["familyGroup"].length > 0 ? data["familyGroup"] : ["Other"]
    );
  };
  ...
  const handleClickFilter = (clickedFilter) => {
    if (activeFilterList.indexOf(clickedFilter) !== -1) {
      setActiveFilterList(
        activeFilterList.filter(function (checkFilter) {
    ...
    push({
      ...
      items: {
        filterName: "Spare parts category",
  ...
  return (
    <div className={styles.bom}>
        ...
        <script type="application/ld+json">{`
              {
                ...
                "itemListElement": [{
                  ...
                  "item": "${baseUrl}/ersatzteile/bom/${normalizeModelNumber(
          modelNumber.toLowerCase()
        )}-${industrialCode.toLowerCase()}"
                }
                ]
              }
            `}</script>
      ...
      <div className={styles.breadcrumbContainer}>
        ...
        {/*  {homeLabel}{" "}*/}
        {/*</a>*/}
        <a className={styles.breadcrumbLink} href="/ersatzteile">
          {" "}
          {sparePartsLabel}{" "}
        </a>
        <div className={styles.breadcrumbLabel}>
          <img
            className={styles.imgBreadCrumb}
            src={
      ...
      {deviceInfo.isMobile && (
        <div className={styles.bom_drawing_info_image_mobile}>
          <div
            className={styles.drawings_info_image_mobile}
            style={{ backgroundImage: `url(${bomDrawingInfoImageMobile})` }}
          ></div>
        </div>
      )}
        ...
        <div className={styles.technical_drawings_drawings_wrapper}>
          {!deviceInfo.isMobile && (
            <div className={styles.bom_drawing_info_image}>
              <div
                className={styles.drawings_info_image}
                style={{ backgroundImage: `url(${bomDrawingInfoImage})` }}
              ></div>
            </div>
          )}
      ...
      <div className={styles.results_wrapper}>
          ...
          {showFilter && (
            <div className={styles.wrapper_selected_category}>
              ...
              {activeFilterList.map((activeFilter) => {
                return (
                  <div
                    className={styles.wrapper_label_filter}
                    onClick={() => handleClickFilter(activeFilter)}
                  >
                    {activeFilter}
              ...
              {filters &&
                ...
                filters.map((filter) => {
                  return (
                    <p
                      className={[
                        activeFilterList.indexOf(filter) !== -1 ||
                          filter === "Other"
                          ? styles.active
                          : styles.filter,
                      ].join(" ")}
        ...
        <div className={styles.results}>
          <h1 className={styles.title_bom_page}>Ersatzteile für Bauknecht: modell {modelNumber} - {industrialCode} </h1>
          <div className={styles.seo_text_bom_page}>
            Bauknecht möchte Ihnen den Einkauf so {randomWord(['angenehm', 'einfach'])} wie möglich machen. Sie finden die  {randomWord(['Ersatzteile', 'Teile'])} für Ihr Modell {modelNumber}. Die Zeichnung hilft Ihnen, das für Ihr {randomWord(['Gerät', 'Produkt', 'Modell'])} {modelNumber} passende Ersatzteil zu finden und Ihr Gerät  {randomWord(['zu reparieren', 'instand zu setzen'])}, damit es wieder {randomWord(['tip top', 'wie am ersten Tag'])} funktioniert. Wir geben 2 Jahr Garantie auf unsere Ersatzteile. Bitte {randomWord(['überprüfen Sie', 'stellen Sie sicher'])} dass dieses {randomWord(['Ersatzteil', 'Teil'])} das {randomWord(['Richtige', 'Korrekte', 'geeignete Teil'])} für Ihr Gerät ist.
          </div>
          ...
          {notFound && <div>{noSparePartsText}</div>}
        </div>
      </div>
            ...
            <div className={styles.modalWrapper}>
              <div className={styles.modal}>
                ...
                <div>
                  ...
                  {spareInBomNotFound && noSparePartsText}
                </div>
              </div>
            </div>
          </div>
