...
import LegalInformative from './components/LegalInformative'
  ...
  switch (action.type) {
    case 'SET_REVIEWS': {
      return {
        ...
        IncentivizedReviewCount: action.IncentivizedReviewCount,
      }
    }
    ...
    case 'SET_LOADED_CONFIG_DATA': {
      return {
        ...state,
        loadedConfigData: action.filteredReviews,
      }
    }
...
const Reviews = ({
  ...
  console.log("GNAAAA", appSettings);
  if (appSettings && product[appSettings?.uniqueId]) productIdentifier = product[appSettings.uniqueId];
    ...
    client
      ...
      .then((response) => {
        const rollup = response?.data?.productReviews?.TotalResults
          ? response?.data?.productReviews?.Includes?.Products?.[0]
              ?.ReviewStatistics
          : null
        ...
        const totalReviews = response?.data?.productReviews?.TotalResults
        const IncentivizedReviewCount =
          response.data?.productReviews?.Includes?.Products?.[0]
            ?.ReviewStatistics?.IncentivizedReviewCount
        ...
        dispatch({
          ...
          IncentivizedReviewCount: IncentivizedReviewCount,
        })
  ...
  if (state.reviews === null) {
    return (
      <div className={styles.loaderFormContainer}>
        ...
        <span className={styles.loaderFormText}>
