...
const SearchReviews: React.FC<SearchReviewsProps> = ({
  ...
  const handleFetch = () => {
    ...
    const fetchUrl = `${BV_API}&passkey=${appSettings.appKey}&locale=de_DE&Filter=ProductId:${productIdentifier}&Search=${inputValue}&limit=${pageSize}&offset=${offset}`
    ...
    fetch(fetchUrl, {
      ...
      .then((response: any) => {
        //GA4FUNREQ35
        push({
          event: 'ga4-reviews-view_search_results',
          type: 'review topic',
          searchResult: response.TotalResults,
  ...
  return (
    <>
      ...
      >
        ...
        {!isSingleReviewOpen && (
          <>
            <h2 className={styles.modalTitle}>Suchergebnisse</h2>
            <div className={styles.modalReviewNumberContainer}>
              <span className={styles.modalReviewNumber}>
                {1 + offset}–{offset + pageSize} wird angezeigt
              </span>
            </div>
          </>
        )}
