...
import { arrowDown, arrowUp } from '../../utils/vectors'
...
const ThumbnailSwiper = props => {
	...
	const arrows = useMemo(() => {
		...
		const upArrow = Buffer.from(arrowUp).toString('base64')
		const downArrow = Buffer.from(arrowDown).toString('base64')

		return (
			<Fragment key="navigation-arrows">
				<span className={nextBtnClassName} style={thumbCaretStyle}>
					<img src={`data:image/svg+xml;base64,${downArrow}`} />
				</span>
				<span className={prevBtnClassName} style={thumbCaretStyle}>
					<img src={`data:image/svg+xml;base64,${upArrow}`} />
				</span>
			</Fragment>
		)
	}, [
