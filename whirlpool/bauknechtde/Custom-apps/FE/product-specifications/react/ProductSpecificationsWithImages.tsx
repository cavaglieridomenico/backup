...
const ProductSpecificationsWithImages: StorefrontFunctionComponent<
	ProductSpecificationsWithImagesProps
> = ({
			...
			{enrichedSpecsArray.map((spec, index) => (
				...
				>
					...
					<div className={styles.left}>
						...
						<p className={styles.title}>{spec && spec.name && spec.name.endsWith(ignorableSuffix) ? spec.name.slice(0, -1*ignorableSuffix.length) : spec.name}</p>
					</div>
...
ProductSpecificationsWithImages.schema = {
	...
	properties: {
		...
		defaultImage: {
			...
			description:
				'Provide a fixed URL to load whenever the images are not provided or fail to load',
			default: '',
			type: 'string',
		},
	},
