...
const BreadcrumbPlp: StorefrontFunctionComponent<BreadcrumbPlpProps> = ({
  BreadcrumbPlpDefault,
}) => {
  const { searchQuery } = useSearchPage();
  const breadPlp = searchQuery?.variables?.selectedFacets[2]?.value.replace(
    /-/g,
    " "
  );
  //console.log("breadPlp", breadPlp);
  return (
    <>{breadPlp ? <div>{breadPlp}</div> : <div>{BreadcrumbPlpDefault}</div>}</>
  );
};
