export async function getRetailers (productSku: string) : Promise<any> {
    const apiUrl = `https://services.internetbuyer.co.uk/REST.svc/GetByProductPartNumber?ProductPartNumber='${productSku}'&CID=175&$format=json` // BK-DE PROD endpoint
    //const apiUrl = `https://services.internetbuyer.co.uk/REST.svc/GetByProductPartNumber?ProductPartNumber='${productSku}'&CID=109&$format=json` // HP-UK PROD endpoint
