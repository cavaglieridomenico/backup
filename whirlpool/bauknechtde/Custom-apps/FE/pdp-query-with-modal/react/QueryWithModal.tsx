...
//console.log(product)
  ...
  const buildUrl = (obj: any) => {
    ...
    return url;*/
    let url = obj["modelNumber"] ? `/ersatzteile/bom/${obj["modelNumber"].replace(/\s+/g, '').replace(/\//g, '').replace(/\(/g, '').replace(/\)/g, '').toLowerCase()}-${obj["industrialCode"]}` : `/ersatzteile/bom/${obj["industrialCode"]}`;
    return url
  }
