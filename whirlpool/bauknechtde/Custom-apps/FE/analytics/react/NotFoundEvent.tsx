...
export const NotFoundEvent: StorefrontFunctionComponent<notFoundEventProps> = ({
  ...
  console.log('in NotFoundEvent of analytics');
  ...
  const getText = (map: string, query: string) => {
    if(map.indexOf('ft') !== -1){ //plain text
      return (
        "/" +
        query.split("/")[
  ...
  useEffect(() => {
    ...
    if (
      ...
      if (searchParameters == "") {
        ...
        // );
        // setnotFound(!notFound);
        const query = searchQuery?.variables !== undefined ? searchQuery.variables.query : NO_QUERY
        const map = searchQuery?.variables !== undefined ? searchQuery.variables.map : NO_MAP
        const isSetMapQuery = map !== NO_MAP && query !== NO_QUERY 
        let filter = isCluster(map) ? createFilterSearch(map,query) : getText(map,query)
