...
const SocialLogin: StorefrontFunctionComponent<SocialLoginProps> = ({
	...
	function putNewSocialUser(sc: string, nOptIn: boolean) {
		const query = new URLSearchParams(window.location.search)
		const fetchUrlPatch =
			"/v1/api/social-connector/oauth/getauthcode?" +
			new URLSearchParams({
				state: query.get("state") || "",
				redirect_uri: query.get("redirect_uri") || "",
				social: sc,
				acceptedTerms: "true",
				isNewsletterOptIn: nOptIn.toString(),
			})
	...
	const handleSubmit = (e: any) => {
		...
		e.preventDefault()
		e.stopPropagation()
		putNewSocialUser(social, newsletterOptIn)
	}
		...
		<div className={style.containerSocialPage}>
			<form onSubmit={handleSubmit}>
				<h3 className={style.title}>
					<FormattedMessage id="store/scelta-social" />
				</h3>
				<div className={style.containerButtonSocial}>
					<Button
						isActiveOfGroup={social === "facebook"}
						onClick={onClickFacebook}
					>
						<span className={style.socialName}>
							<FormattedMessage id="store/facebook" />
						</span>
						<img src={iconFacebook} width="24px"></img>
					</Button>
					<Button isActiveOfGroup={social === "google"} onClick={onClickGoogle}>
						<span className={style.socialName}>
							<FormattedMessage id="store/google" />
						</span>
				...
				<div className={style.containerTermini}>
					<label>
						<FormattedMessage id="store/intro-informativa" />
						<a
							className={style.testoInformativa}
							href="/pagine/informativa-sulla-privacy"
						>
							<FormattedMessage id="store/informativa" />
						</a>{" "}
						e:
					</label>
					<div className={style.checkboxContainer}>
						...
						<label className={style.newsletter}>
							<FormattedMessage id="store/newsletter" />
						</label>
					</div>
				</div>
				<div className={style.containerButtonSocial}>
					{!loading ? (
						<Button type="submit" disabled={social === ""}>
							<span>
								<FormattedMessage id="store/registrati" />
							</span>
						</Button>
					) : (
