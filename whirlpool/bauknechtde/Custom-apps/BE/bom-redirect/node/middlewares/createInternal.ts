...
export async function createInternal(ctx:Context, next: () => Promise<any>) {
  ...
  let route: Route = {
    from: `/ersatzteile/bom/${modelNumberUrl}-${industrialCodeUrl}`,  //  /ersatzteile/bom/he62fixha-aq555800000
    resolveAs: "/bom",
    declarer: DECLARER,
    type: TYPE,
  ...
  try{
    ...
    ctx.status = 400
    ctx.body = {
      message: "Error creating internal for url: " + "/ersatzteile/bom/" + modelNumberUrl + "-" + industrialCodeUrl
    }
  }

  ctx.status = 200
  ctx.body = {
    message: " Internal created for url: " + "/ersatzteile/bom/" + modelNumberUrl + "-" + industrialCodeUrl
  }
}
