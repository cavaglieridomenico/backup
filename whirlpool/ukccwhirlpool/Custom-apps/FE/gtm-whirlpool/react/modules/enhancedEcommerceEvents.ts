...
import { getVipCompany } from "../utils/utilityFunctionPageView"
...
export async function sendEnhancedEcommerceEvents(e: PixelMessage) {
  const tradePolicy = window.location.href.includes("epp")
    ? "EPP"
  ...
  switch (e.data.eventName) {
    ...
    case "vtex:productView": {
      ...
      push({
        ...
        contentIndex: document.getElementsByClassName(
          "ukccwhirlpool-video-player-thron-pdp-0-x-thumbImg--productPage"
        ).length,
      })
              ...
              (response) => {
                setTimeout(() => {
                  const list: string = getListProductClickAndDetail(
                    selectedSku?.name,
                    response?.AdWordsRemarketingCode
                  )
                  const pr = {
                    ecommerce: {
                      currencyCode,
        ...
        ecommerce: {
          add: {
            ...
            products: items.map((sku: any) => ({
              ...
              dimension5: dim5Value,
              dimension6:
                skuItem.bestPrice === skuItem.listPrice
                  ? "Not in promo"
                  : "In promo",
            })),
          },
    ...
    case "vtex:removeFromCart": {
      ...
      let dim5Value = costructionType(findDimension(data, "constructionType"))
      let dim4Value =
        findDimension(data, `sellable${tradePolicy}`) == "true"
          ? "Sellable Online"
          : "Not Sellable Online"
      let productAPI = await getCategoryFromIdProduct(items[0].productId)
      let values = await getStringCategoryFromId(productAPI.CategoryId)
    ...
    case "vtex:productComparison": {
      ...
      const hasMultipleCategories = productsInfo.some(
        (o: any) => o.categoryId != productsInfo[0].categoryId
      )
      const productsCodes = productsInfo
        .map((product: any) => product.productReference)
        .join(", ")
      if (!hasMultipleCategories) {
        const categoryStringValue = await getStringCategoryFromId(
          productsInfo[0].categoryId
        )
        const macroCategory = getMacroCategory(
          categoryStringValue.AdWordsRemarketingCode
        )
        objsToPush.push({
        ...
        const categoryIds = productsInfo.map(
          (product: any) => product.categoryId
        )
        const uniqueCategories = [...new Set(categoryIds)]
        const categoriesDetails = await Promise.all(
          uniqueCategories.map((cat: any) => getStringCategoryFromId(cat))
        )

        const productCategory = categoriesDetails.map(
          (cat) => cat.AdWordsRemarketingCode
        )
        const macroCategory = productCategory.map((cat) =>
          getMacroCategory(cat)
        )
    ...
    case "vtex:personalArea": {
      const { section, isLogoutOk } = e.data
      push({
        event: "personalArea",
        eventCategory: "Personal Area",
        eventAction: section,
        eventLabel: `${section}: ${isLogoutOk ? "ok" : "ko"}`,
      })
      break
    }
...
function getListProductClickAndDetail(productId: string, category: string) {
  ...
  // In case user clicks on product card before `eec.impressionView` analytics event
    ...
    let lastPageViewDetails =
      getLastDetailOrCategoryPageViewDetails("productClick")
    let lastPageViewDetailsisAccessories = lastPageViewDetails.category
      ? lastPageViewDetails.category.includes("_AC_") ||
        lastPageViewDetails.macroCategory == "AC"
      : false
    let isProductAccessory = productCategory
      ? productCategory.includes("_AC_")
      : false
    ...
    if (previousPageType === "home") {
      ...
      lastPageViewDetailsisAccessories
    ) {
...
async function getProductsFromOrderData(
  data: any,
  ...
  await Promise.all(
    productsFromOrder.map(async (value: any) => {
      ...
      var obj = {
        ...
        dimension4:
          getValuefromSpecifications(values[0], `sellable${tradePolicy}`) ===
          "true"
            ? "Sellable Online"
            : "Not Sellable Online",
      ...
      ecommerce: {
        ...
        purchase: {
          actionField: {
            ...
            affiliation: `Whirlpool UK ${tradePolicy}`,
...
function getLastDetailOrCategoryPageViewDetails(eventType: string) {
  let pageViews = window.dataLayer.filter(
    (cat) =>
      cat.event == "pageView" &&
  ...
  let lastEventDetails = {
    category: pageViews[pageViews.length - 1]
      ? pageViews[pageViews.length - 1]["product-category"]
      : "",
    macroCategory: pageViews[pageViews.length - 1]
      ? pageViews[pageViews.length - 1]["product-macrocategory"]
      : "",
    type: pageViews[pageViews.length - 1]
      ? pageViews[pageViews.length - 1]["pageType"]
      : "",
  }
