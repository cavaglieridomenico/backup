  ...
  return (
    <>
      ...
      {prop && prop.slots?.length > 0 && !shipAllProductTogether && <div className={styles.customdate__wrapper}>
        <p className={styles.shiptext}>{customShippingLabel ? customShippingLabel : <FormattedMessage id="checkout-hdx-delivery-slots-uk.preferred-delivery-date" />}</p>
          ...
          {slots && slots.length > 0 &&
            slots.map((slot: any, index: number) => {
              return <option key={index} value={index} selected={slot.active}>
                {slot.weekday} {slot.dayNumber.replace(/^0+/, '')} {slot.month} ({slot.price && slot.price})
              </option>
            })
          }
        </select>
