...
const ShippingForm: StorefrontFunctionComponent<ShippingFormProps> = ({
  /*   customShippingLabel = "",
    customAccessoriesShippingLabel = "" */
}) => {
  const intl = useIntl()
  const { push } = usePixel();
  ...
  const handleSubmit = async (e: any) => {
    ...
    if (formData.postCode == "" || formData.street == "" || formData.town == "" || (slotData.length > 0 ? activeSlot.length <= 0 : false)) {
      setSubmitError({
      ...
      {isPostCodeValid && <div className="mb5">
        ...
        <div className="mb5">
          <Input
            ...
            id="town"
            onChange={(e: any) => { handleChangeInput(e) }}
            errorMessage={(submitError.town && !formData.town) ? intl.formatMessage(messages.inputError) : ""}
          />
        </div>
        <div className="mb5">
          <Input
            ...
            id="county"
            onChange={(e: any) => { handleChangeInput(e) }}
          /*errorMessage={(submitError.county && !formData.county) ? intl.formatMessage(messages.inputError) : ""}*/
          />
        </div>
        <div className="mb5">
...
ShippingForm.schema = {
  title: "HDX custom app schema",
  description: "Here you can chose if you want to show a customized label for scheduled shipping",
  type: "object",
  properties: {
    customShippingLabel: {
      title: "Text for scheduled shipping",
      default: "",
      type: "string",
    },
    customAccessoriesShippingLabel: {
      title: "Text for accessories shipping",
      default: "",
      type: "string",
    },
  },
