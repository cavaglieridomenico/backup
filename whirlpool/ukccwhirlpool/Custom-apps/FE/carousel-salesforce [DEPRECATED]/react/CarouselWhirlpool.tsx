...
function CarouselWhirlpool({
  ...
  const getProducts = async () => {
    ...
    return await fetch("/app/sfmc/recommendations" + HOST, options)
  ...
  const productList = (ids: any) => {
    ...
    if (error) {
      console.error("[SLFC] Error! ", error);
      return null;
    }
      ...
      <>
        {!error && data && (
          <ProductListProvider listName={listName ?? ""}>
            <div>
              <span className={style.carouselTitle}>{carouselTitle}</span>
              <List
...
CarouselWhirlpool.schema = {
  title: "Salesforce Carousel",
  description: "Salesforce carousel props",
  type: "object",
  properties: {
    carouselTitle: {
