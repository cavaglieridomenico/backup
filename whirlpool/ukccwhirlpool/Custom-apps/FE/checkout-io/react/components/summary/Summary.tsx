...
import { Divider, IconArrowBack, IconCheck, ButtonPlain } from "vtex.styleguide"
...
const CheckoutSummary: React.FC<CheckoutSummaryProps> = ({
	...
	const setProperLabel = (id: any, leadtime: any) => {
		if(specifications && tradePolicy){
			...
			let inStockAndSellableSpecs = itemSpecs[0]?.properties?.filter((item: any) => item?.name?.includes("stockavailability") || item?.name?.includes("sellable"))
			let specsByTradePolicy = inStockAndSellableSpecs?.filter((item: any) => item.name.includes(tradePolicy))
			if(specsByTradePolicy && specsByTradePolicy?.length > 0){
				...
				let inStock = specsByTradePolicy?.filter((item: any) => item.name.includes("stockavailability"))[0]?.values[0]
				if(sellable == "true"){
									...
									<div className={`${style.description} ph4 c-on-base`}>
										...
										<div className="flex items-center">
											{showEnergyAndSheet && item.EnergyLogoImage && (
												<a href={item.energyLabel} target="_blank" className="mr3">
					...
					{totalizers?.map(
						(total: Totalizer, i: number) =>
							total.id != "Shipping" ? (
								...
								<div key={i} className="flex flex-row justify-between">
									<p className={style.totalizerTitle}>{intl.formatMessage(messages.shipping)}</p>
									<span className="flex items-center">
											{total?.value == 0 ? intl.formatMessage(messages.freeLabel) : <FormattedPrice value={Number(total?.value) / 100} />}
									</span>
								</div>)
					)}
				</div>
				...
				<div className={`${style.summaryTextAreaContainer}  t-small`}>
					<span>
						<FormattedMessage
							...
							defaultMessage="Personalized message (max {variable} characters)"
...
const messages = defineMessages({
	...
	openTextLabel: {
		defaultMessage: "Personalized message (max {characters} characters)",
		id: "checkout-io.openText",
	},
	freeLabel: {
		defaultMessage: "Free",
