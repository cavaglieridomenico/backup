...
const ProductQuantity: React.FC = () => {
	...
	const sendEecAddToCartEvent = () => {
		push({ event: "eec.addToCart", item })
	}
	const sendEecRemoveFromCartEvent = () => {
		push({ event: "eec.removeFromCart", item })
	}
	/* const sendCartStateEvent = () => {
		setTimeout(() => {
			if (quantityLoading == false) push({ event: "cartState", cartOrderForm })
		}, 1000)
	} */
	useEffect(() => {
		if (data != undefined) {
			push({ event: "cartState", cartOrderForm })
		}
	}, [item.quantity])
