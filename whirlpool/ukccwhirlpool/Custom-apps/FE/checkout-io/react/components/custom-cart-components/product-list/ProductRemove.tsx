...
const ProductRemove: React.FC = () => {
	...
	console.log(cartOrderForm?.items?.length, "cartOrderForm?.items?.length")
	...
	const sendEecRemoveFromCart = () => {
		push({ event: "eec.removeFromCart", item, sellableField5AndVariant })
	}
				...
				<button
					onClick={() => {
						quantityMutation(item.id, itemIndex, 0),
							cartOrderForm?.items?.length == 1 && window?.location?.reload()
