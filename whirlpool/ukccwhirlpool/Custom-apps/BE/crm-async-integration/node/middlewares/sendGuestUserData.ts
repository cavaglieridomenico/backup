...
export async function sendGuestUserData(ctx: Context|OrderEvent, next: () => Promise<any>) {
  ...
  try{
    if(ctx.state.appSettings.checkoutAsGuest){
      ...
      let email: string = order.customData?.customApps?.find(f => f.id==CustomApp.PROFILE)?.fields[ProfileCustomFields.email]?.toLowerCase();
      ...
      if(userData?.email?.toLowerCase()!=email && optin){
        ...
        .catch(err => ctx.vtex.logger.error("Guest user registration (order id: "+orderId+"): failed --err: "+JSON.stringify(err)+" --data: "+JSON.stringify(payload)))
      }
    }
    ctx.status = 200;
    ctx.body = "OK"
