...
function FuncCloseMenuMouseoutCooking(ref) {
    if (canUseDOM) {
        useEffect(() => {
            const menu = document.getElementsByClassName("vtex-menu-2-x-submenu--menu")[1];
            function handleCloseMenu(event) {
                if (ref.current && !ref.current.contains(event.target)) {
                    const outerMenu = document.getElementsByClassName("vtex-menu-2-x-submenuWrapper--menu")[1];
