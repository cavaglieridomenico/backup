...
const Filters: StorefrontFunctionComponent<FilterProps> = ({
  ...
  useEffect(() => {
    ...
    const temp = data.queryRecipes.facets
      .filter(item => !['mainIngredients', 'idType'].includes(item.name))
      .map((facet: any) =>
        facet.values.map((value: any) => {
          const isImage = data.queryRecipes.facetsId[
            data.queryRecipes.facetsId.findIndex(item => item.name == value)
          ]?.id
            ? true
            : false

          const imageId = data.queryRecipes.facetsId.find(
          ...
          return {
            ...
            name: value,
            value: false,
            image: icons[imageId] || undefined,
            style: isImage ? styles.imageFilter : styles.filterCourses,
            disabled: false,
            disabledStyle: isImage
              ? styles.imageFilterDisabled
              : styles.filterDisabled,
            activeStyle: isImage
              ? styles.imageFilterActive
              : styles.filterCoursesActive,
            defaultStyle: isImage ? styles.imageFilter : styles.filterCourses,
            field: facet.name,
            single: isImage,
          }
        })
  ...
  const handleFilterChange = (
    ...
    const filter = internalFilters[indexCategory][indexFilter]
    const filterCategory = internalFilters[indexCategory]?.name

    if (!filter) return temp

    filter.value = value ? value : !filter.value
    filter.style = filter.value ? filter.activeStyle : filter.defaultStyle
    temp[indexCategory][indexFilter] = filter

    filtersPush(filter.id, filterCategory)

    if (filter.single) {
      temp[indexCategory].forEach((filter, index) => {
        if (index !== indexFilter) {
          filter.value = false
          if (filter.disabledStyle === filter.style) {
            filter.style = filter.disabledStyle
          } else if (filter.activeStyle === filter.style) {
            filter.style = filter.activeStyle
          } else {
            filter.style = filter.defaultStyle
          }
        }
      })
    }
    ...
    temp.forEach(category => {
      category.forEach(filter => {
        const check = results.some(result => {
          return result[filter.field] === filter.id
        })
        if (!check) {
          filter.style = filter.disabledStyle
  ...
  useEffect(() => {
    ...
    const newFilters: Record<any, any[]> = {}

    const temp = [...internalFilters]
    temp.forEach(category => {
      category.forEach(filter => {
        filter.value = query['filters[]'].includes(filter.id)
        if (filter.value)
          newFilters[category.field] = [
            ...(newFilters[category.field] || []),
            filter.id,
          ...
          internalFilters.map((category, indexC) => (
            <React.Fragment key={indexC}>
              <div className={styles.categoryFiltersContainer}>
                {category.map((filter, indexF) => (
                  <div
                    className={filter.style}
                    key={filter.id}
                    onClick={() => handleFilterClick(indexC, indexF)}
                  >
                    {icons[filter.idIcon] && icons[filter.idIcon]}
                    <span
                      className={
                        icons[filter.idIcon]
                          ? styles.imageFilterText
                          : styles.filterCourserText
                      }
                    >
                      {filter.name}
                    </span>
              ...
              {internalFilters.length - 1 !==
                internalFilters.indexOf(category) && (
