...
const messages = defineMessages({
  filterFor: { id: 'store/faq.filterFor' },
  readMore: { id: 'store/faq.readMore' },
})
...
const DomandeFrequenti: StorefrontFunctionComponent<DomandeFrequentiProps> = () => {
  ...
  useEffect(() => {
    ...
    fetch(
      ...
      .then(json => {
        if (json.length) {
          let filters = ['Wszystko'] as string[]
  ...
  const filterQuestionsFirstLevel = (filter: string) => {
    ...
    filterQuestions(filter,'Wszystko')
  }

  const filterQuestions = (filterFirstLevel:string, filter: string) => {
    setSelectedFilter(filter)
    if (filterFirstLevel == '' && filter === 'Wszystko') {
      setQuestion(question2)
    } else if(filterFirstLevel == '' && filter !== 'Wszystko'){ 
  ...
  const content = (
    <>
      <span className={classnames(styles.filterPer)}>{intl.formatMessage(messages.filterFor)}</span>
          ...
          <>
            ...
            <div className={classnames(styles.domandaContainer, 'w100')}>
              ...
              >
                ...
                <a className={classnames(styles.readMore)}>{intl.formatMessage(messages.readMore)}</a>
              </Link>
            </div>
          </>
        ))}
