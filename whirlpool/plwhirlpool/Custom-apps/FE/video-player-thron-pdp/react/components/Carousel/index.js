...
//     let idVideo = url
//       .split("vi-")[1]
//       .substr(0, url.split("vi-")[1].indexOf("/"));
...
function pushAfterXsec(dataLayer, seconds) {
  return (
    dataLayer[dataLayer.length - 1].eventAction !== undefined &&
    dataLayer[dataLayer.length - 1].eventAction.indexOf(`${Math.round(seconds)} secs`) !== -1
  );
}
...
class Carousel extends Component {
      ...
      if (this.props.slides[activeIndex].type == "video") {
        ...
        if (vid !== undefined) {
          ...
          vid.ontimeupdate = function () {
            let secs = Math.round(vid.currentTime)
            if ([10, 25, 50, 75, 90].includes(secs)) {
              // let final = activeSlide.src.split("WEBHD/vi-");
              // const idVideo = final[1];
              //let dataLayer = window.dataLayer;
              if (!pushAfterXsec(dataLayer, vid.currentTime)) {
                ...
                //   eventCategory: "Product Experience",
                //   eventLabel: "Product Experience Video - " + idVideo,
                //   eventAction: `View a Video - ${secs} secs`,
                // });

                //GA4FUNREQ64
                window.postMessage({ ...ga4Data, actionType: `${secs} secs` }, window.origin);
  ...
  render() {
    ...
    const renderSlides = () => {
      ...
      } catch (e) { console.log(e, "SWIPER RENDER GONE WRONG") }
    }

    return (
      <>
        {isModalState && modalSwiper}
        {this.props.showImagesCounter && <span className={styles.imagesCounterText}>{`${this.state.activeIndex + 1}/${slides?.length}`}</span>}
