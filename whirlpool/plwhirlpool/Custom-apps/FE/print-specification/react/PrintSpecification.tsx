...
const PrintSpecification: StorefrontFunctionComponent<PrintSpecificationProps> = ({
  ...
  let availableQuantity: any =
    productContextValue?.selectedItem?.sellers[0]?.commertialOffer
      ?.AvailableQuantity;
  let minimumQuantityThreshold: any = 0;
  productContextValue?.product?.specificationGroups?.forEach(
    (specification: any) => {
      let specFilter = specification?.specifications?.filter(
        (spec: any) => spec?.name === "minimumQuantityThreshold"
      );
      if (specFilter?.length > 0 && !minimumQuantityThreshold)
        minimumQuantityThreshold = specFilter[0]?.values[0];
    }
  );
        ...
        isLink ? (
          <div className={style.container}>
            <p className={style.attribute}>
              ...
              > */}
                {/* {e.attribute === "DISPONIBILITÀ" &&
                e.value === "Whirlpool Online Shop" &&
                availableQuantity > 0 &&
                ...
                {e.attribute === "Do kupienia online" &&
                e.value === "Sklep whirlpool.pl" &&
                availableQuantity > 0 &&
                availableQuantity >= minimumQuantityThreshold ? (
                  <span className={style.value + ' ' + style.dot}>Do kupienia online</span>
                ) : e.attribute === "Do kupienia online" ? (
                  <span className={style.valueNotAvailable + ' ' + style.dot}>
