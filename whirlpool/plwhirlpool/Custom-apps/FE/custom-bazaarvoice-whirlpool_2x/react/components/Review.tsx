...
const messages = defineMessages({
  ...
  timeAgoMonthsMore: {
    id: 'store/bazaar-voice.timeAgo.monthsMore',
    defaultMessage: 'months',
  },
  timeAgoWeek: {
  ...
  if (months > 0) {
    const message =
      months > 1 && months < 5
        ...
        : months >= 5
        ? intl.formatMessage(messages.timeAgoMonthsMore)
        : intl.formatMessage(messages.timeAgoMonth)
    return `${months} ${message} ${intl.formatMessage(messages.timeAgo)}`
  }
...
const Review: FunctionComponent<ReviewProps> = ({
  ...
  const [state, setState] = useState<any>({
    isSyndicated: review.IsSyndicated
      ? review.IsSyndicated.toLowerCase() === 'true'
      : null,
  ...
  return (
    ...
    >
      <ReviewStructuredData
        ...
        productId={product.productId}
        productUrl={'https://www.whirlpool.pl/' + product?.linkText + '/p'} //to fix the vtex url generation which was returning https://portal.vtexcommercestable.com...
        review={review}
      />
      ...
      <div className={`${styles.titleContainer}`}>
        ...
        {isIncentivized && (
          <span className={styles.incentivedReview}>
            <img src={icon} className={styles.imageReviewIncentived} />
