...
export async function returnOrder(ctx: Context, next: () => Promise<any>){
    ...
    try{
      ...
      let res = await triggerRefundOrReturnEmail(ctx, JSON.parse(process.env.SFMC+"").returnKey, product, 2, tokenCredential);
      ctx.status = 200;
      ctx.body = "OK";
      ...
      if(e.response==undefined){
        ctx.vtex.logger.error(e.message!=undefined?e.message:"Internal Server Error");
      }else{
        ctx.vtex.logger.error("Return "+product.ContactAttributes?.SubscriberAttributes?.OrderNumber+" - app settings: "+e.response?.data?.message!=undefined?e.response?.data?.message:"Internal Server Error");
      }
    }*/
    ctx.status = 404;
