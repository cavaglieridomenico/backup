...
export async function createOrder(ctx: Context, next: () => Promise<any>){
    ...
    if(credentials.find(f => f.key==ctx.req.headers[("X-VTEX-API-AppKey").toLowerCase()] && f.token==ctx.req.headers[("X-VTEX-API-AppToken").toLowerCase()])!=undefined){
      if(ctx.query.id!=undefined && ctx.query.id!=null && ctx.query.id!=""){
        try{
          ...
          let res = await createOrCancelOrder(ctx, servicesName, JSON.parse(process.env.SFMC+"").orderDetailsKeyConf, JSON.parse(process.env.SFMC+"").emailTriggerKeyConf, 1, ctx.query.id, tokenCredential, baseURL);
          ctx.status = 200;
          ctx.body = "OK";
          ...
          if(e.response==undefined){
            ctx.vtex.logger.error(e.message!=undefined?e.message:"Create "+ctx.query.id+": Internal Server Error");
          }else{
            ctx.vtex.logger.error("Create "+ctx.query.id+" - app settings: "+e.response?.data?.message!=undefined?e.response?.data?.message:"Internal Server Error");
          }
        }
      }else{
        ctx.status = 400;
