...
export async function GetPayment(ctx: Context, key: string, timeout = 2000, maxRetries = 1, keyfield = PaymentKeys.ORDERID, currentRetry = 0): Promise<PaymentMD> {
  ...
  return new Promise(async (resolve, reject) => {
    ...
    if (existingPayment == null || existingPayment == undefined) {
      logger.info("[GetPayment] order:" + key + " - Payment not found in Vbase")
      console.log("payment not found in cache, order: " + key)
      ctx.clients.masterdata.searchDocuments<PaymentMD>({
        ...
        if (results.length > 0) {
          console.log("payment found in MD")
          logger.info("[GetPayment] order:" + key + " - Payment found in MD")
          resolve(results[0])
        } else {
          console.log("payment not found in MD, retrying")
          logger.info("[GetPayment] order:" + key + " - Payment not found in MD")
      ...
      logger.info("[GetPayment] order:" + key + " - Payment found in Vbase ")
      console.log("payment found in cache, order: " + key)
