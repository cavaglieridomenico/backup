        ...
        if (Object.keys(items[index].productCategories).indexOf("2") == -1) {
          ...
          const tradePolicy =
            vtexjs.checkout.orderForm.salesChannel == 1 &&
              window.location.hostname.includes("epp")
              ? "EPP"
              : vtexjs.checkout.orderForm.salesChannel == 2
                ...
                : vtexjs.checkout.orderForm.salesChannel == 3
                  ? "VIP"
                  : "";
          const filteredServices = item.offerings.filter(
            (offering) => tradePolicy == "" && !offering.name.includes("_")
          );
          const availableServices = sortServices(filteredServices);
          console.log(item.offerings, "item.offerings");
          ...
          if (viewportWidth > 640) {
            ...
            <div class="flexw100">
              <i class="icon-ok preselected-service-icon"></i>
              <span class="preselected-service-name">Livraison à domicile</span>
              <i class="icon-question-sign" data-tooltip="tooltipModal-preselected" onmouseover="handleMouseover()"></i>
          ...
          availableServices.forEach((service) => {
            service.name = service.name.includes("_")
              ...
              if (serviceConfig.selectionEnabled) {
                ...
                if (viewportWidth > 640) {
                  ...
                  <div id='${service.id}' class='extra-services-checkbox'>
                    <div class='extra-services-label-tooltip'>
                      <label class='extra-services-label' for='${index}-${service.id}'>
                      <input type='checkbox'
                        id='${index}-${service.id}'
                        class='extra-services-input'
                      ...
                      <div class="tooltip">
                        <i class="icon-question-sign" onmouseover="handleMouseover()"></i>
                        <span class="tooltiptext">${serviceConfig.description}</span>
                      </div> 
                    </div>
                    <span class='extra-services-price'>${servicePrice}</span>
                  </div>`;
                } else {
                  selectableElements += `<div id='${service.id
                    ...
                    <i class="icon-question-sign" data-tooltip="tooltipModal-${service.id
          ...
          if (viewportWidth > 640) {
            parentElement.append(
              ...
              <p class='title-custom-extra-services'><i class="icon-plus"></i>Vos Services</p>
               ${selectableElements}
               <div class='title-custom-extra-services-container'>
                <p class='title-custom-extra-services extended-warranty-options'>Vos Garanties</p>
               </div>
...
function createExtraServicesList() {
  ...
  const formatServicesPrice = (price) =>
    price == 0
      ? "Inclus"
      : `${(price / 100).toFixed(2).replace(".", ",")} ${vtexjs.checkout.orderForm.storePreferencesData.currencySymbol
      }`;
