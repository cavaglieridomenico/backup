...
const Reviews = ({
  ...
  // const productName = product.items[0].name
  ...
  // let skuName = appSettings.uniqueId === 'skuName' ? true : false

  // const productIdentifier = product[appSettings.uniqueId]
  const productIdentifier = product.items[0].name
  ...
  useEffect(() => {
    ...
    client
      ...
      .catch((error) => {
        console.error('Bazaarvoice', error)
        dispatch({
          type: 'QUERY_ERROR',
        })
      })
  ...
  return state.reviews.length ? (
    ...
    >
      <p className={`${styles.reviewsTitle} t-heading-3 b--muted-5 mb5`}>
