...
export async function CapturePayment(ctx: Context, next: () => Promise<any>) {
  ...
  const existingPayment: PaymentMD | undefined = await GetPayment(ctx, capturePaymentRequest.paymentId, 0, 0, PaymentKeys.PAYMENTID).catch(() => undefined)
    ...
    if (existingPayment.status != PaymentStatusVtex.captured) {
      if (appSettings.useXipay) {
                ...
                capture: {
                  ...
                  amount: {
                    $: {
                      ...
                      currencyCode: capturePaymentRequest.currency ?? appSettings.defaultCurrencyCode,
                      exponent: 2,
        ...
        if (jsonRes && jsonRes.paymentService?.reply != undefined && jsonRes.paymentService?.reply[0].ok != undefined) {
          ...
          SavePayment(ctx, existingPayment.orderid, existingPayment, true).catch(err => {
            logger.error("[Capture Payment] order:" + existingPayment.orderid + " - Error updating master data")
            logger.debug(err)
          })
        } else {
          capturePaymentResponse.code = jsonRes?.paymentService?.reply?.[0]?.error?.[0]?.$?.code || "-1"
          capturePaymentResponse.message = jsonRes?.paymentService?.reply?.[0]?.error?.[0]?._ || "Unexpected error"
          ctx.status = 500;
        }
      }
