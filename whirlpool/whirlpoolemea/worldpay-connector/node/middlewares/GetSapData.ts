...
import { Acquirer, authHeaders, AUTH_FLAG, CC_REACT, CC_STAT_EX, DATAORIGIN, PaymentStatusVtex } from "../utils/constants"
...
export async function GetSapData(ctx: Context, next: () => Promise<any>) {
  ...
  if (payment == undefined || payment.status != PaymentStatusVtex.approved || payment.paymentMethod == "banktransfer") {
    if (payment !== undefined && payment.paymentMethod == "banktransfer")
      logger.warn("[SAP data] order:" + orderid + " - Payment with PRZELEWY24, no SAP integration")
    else {
