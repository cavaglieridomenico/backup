...
import { defaultWPpaymentRequest, orderSuffix } from "../../utils/constants";
...
export abstract class  AbstractWpRequestBuilder {
    ...
    protected wpRequest! : WPCreatePaymentRequest;
    protected order!: Order;

   
    async buildRequest(createPaymentRequest: CreatePaymentRequest, _ctx : Context, appSettings: configs): Promise<WPCreatePaymentRequest> {
        
        // creating the requests from a template
        this.wpRequest = JSON.parse(JSON.stringify(defaultWPpaymentRequest))    
