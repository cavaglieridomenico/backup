...
export default class CronJob extends ExternalClient {
  ...
  private async createCronJob(cronBody: CronBody, ctx: Context | InstalledContextEvent) {
    return new Promise<any>((resolve, reject) => {
      this.http.post('', cronBody, {
        ...
        .then(res => {
          ctx.state.logger.debug(`🚀 ~ file: CronJob.ts:50 ~ CronJob ~ createCronJob ~ res: ${stringify(res)}`)
          resolve(res)
        })
  ...
  public async createCronIfNotExists(uri: string, cronBody: CronBody, ctx: Context | InstalledContextEvent) {
    return new Promise<boolean>(async (resolve, reject) => {
      try {
        const crons: CronResponse[] = await this.getCronJobs();
        const isActiveCron = crons.find((cron) => cron.request.uri.includes(uri));
        if (!isActiveCron) {
          await this.createCronJob(cronBody, ctx);
        }
