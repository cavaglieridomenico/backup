...
const ShippingOption: React.FC<Props> = ({
	...
	useLayoutEffect(() => {
		if (
			...
			console.log("Suddenly calling onSelectOption")
			console.log("hasSchedule: ", hasSchedule)
			onSelectOption(
				index,
				"Appuntamento",
				hasSchedule?.[0]?.availableDeliveryWindows.filter(
					window => window.price == 0,
				)?.[0],
			)
		}
	...
	const handleDropdownDayChange: React.ChangeEventHandler<HTMLInputElement> = event => {
