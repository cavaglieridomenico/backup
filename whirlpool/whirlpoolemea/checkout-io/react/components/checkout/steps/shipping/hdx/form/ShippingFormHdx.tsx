...
import {
	...
	options,
	state,
} from "./utils/utilsForShippingForm"
...
const ShippingForm: FC<ShippingFormProps> = ({ showShipTogheter = true }) => {
	const intl = useIntl()
	const { push } = usePixel()
	const { authCookie } = useCheckout()
	...
	const [
		...
		setShowRemoveInstallationModal,
	] = useState(false)
	...
	const shippingPrice =
		orderForm?.shippingData?.logisticsInfo[0]?.slas[0]?.price
	...
	const logisticInfos: any = orderForm?.shippingData?.logisticsInfo
	...
	logisticInfos?.forEach((li: any) => {
		if (
			...
			!itemsTypes.SDA &&
			li.slas.filter((sla: any) => sla.id === shippingPolicies.SPECIAL).length >
				0
		) {
	...
	if (
		orderForm?.customData?.customApps?.filter(
			(app: any) => app.id == "tradeplace",
		)[0]?.fields.connectedGas == "true"
	)
	...
	const handleChangeInputAddresses = (e: any) => {
		...
		if (e.target.value !== "") {
			...
			setStreetOptions(
				tempStreetOptions.filter((address: any) =>
					address.label.toLowerCase().includes(e.target.value.toLowerCase()),
				),
			)
		} else {
			setStreetOptions(formatAddresses(addresses))
	...
	useEffect(() => {
		if (selectedAddress) {
			setFormData({
				...
				houseNumber: selectedAddress.complement
					? selectedAddress.complement
					: "",
				town: selectedAddress.city ? selectedAddress.city : "",
				county: selectedAddress.state ? selectedAddress.state : "",
				stairs: selectedAddress.reference
					? Boolean(selectedAddress.reference)
					: false,
			})
		}
		if (
			selectedAddress?.postalCode &&
			isValidPostcode(selectedAddress.postalCode)
		) {
	...
	const loadDeliverySlots = async (newOrderForm: any) => {
		...
		if (isPostCodeValid) {
			...
			await fetch(`/app/hdx/delivery/slots/${orderFormId}`)
				...
				.then(data => {
					let tradeplaceData = newOrderForm
						? newOrderForm?.customData?.customApps?.filter(
								(app: any) => app.id == "tradeplace",
						  )[0]?.fields
						: orderForm?.customData?.customApps?.filter(
								(app: any) => app.id == "tradeplace",
			...
			variables: {
				...
				address: {
					postalCode: postalCode
						? filterPostalCodeUK(postalCode)
						: filterPostalCodeUK(formData.postCode),
					country: "GBR",
					receiverName:
	...
	const [postalCodeServed]: any = useMutation(isServedPC, {
		onCompleted(data) {
			if (data && data["isServedPC"]) {
				if (
					!data["isServedPC"]["isServed"] &&
					data["isServedPC"]["removedItems"] > 0
				) {
	...
	const loadAddressPTV = async (postalCode: string, hash: string) => {
		if (
			(hash === "#/shipping" &&
				postalCode &&
				postalCode !== state.hotpointPTV.lastPostalCode &&
				!state.hotpointPTV.inProgress) ||
	...
	useEffect(() => {
		;(Object.values(submitError) as boolean[]).forEach(
			(error: boolean) =>
				error &&
				push({
	...
	const handleSubmit = async (e: any) => {
		...
		if (formData.postCode == ""  || formData.street == "" || formData.town == "" || formData.town.includes("\x01") || formData.town.includes("\x02") || formData.town.includes("\x03") || formData.town.includes("\x04") || formData.town.includes("\x05") || formData.town.includes("\x06") || formData.town.includes("\x07") || formData.town.includes("\x08") || formData.town.includes("\x09") || formData.town.includes("\x0A") || formData.town.includes("\x0B") || formData.town.includes("\x0C") || formData.town.includes("\x0D") || formData.town.includes("\x0E") || formData.town.includes("\x0F") || formData.houseNumber.includes("\x01") || formData.houseNumber.includes("\x02") || formData.houseNumber.includes("\x03") || formData.houseNumber.includes("\x04") || formData.houseNumber.includes("\x05") || formData.houseNumber.includes("\x06") || formData.houseNumber.includes("\x07") || formData.houseNumber.includes("\x08") || formData.houseNumber.includes("\x09") || formData.houseNumber.includes("\x0A") || formData.houseNumber.includes("\x0B") || formData.houseNumber.includes("\x0C") || formData.houseNumber.includes("\x0D") || formData.houseNumber.includes("\x0E") || formData.houseNumber.includes("\x0F") || (slotData.length > 0 ? activeSlot.length <= 0 : false)) {
			...
			let logisticsInfo: any = [...orderForm?.shippingData?.logisticsInfo]
			if (selectedAddresses && logisticsInfo) {
				...
				selectedAddresses[0].postalCode = filterPostalCodeUK(
					orderForm?.shippingData?.address?.postalCode || formData.postCode,
				)
				...
				const updatedLogisticsInfo = logisticsInfo.map((logInfo: any) => {
					...
					if (
						logInfo.selectedSla === "Standard free" ||
						logInfo.selectedSla === "Standard charged" ||
						logInfo.selectedSla === "Second day delivery" ||
						logInfo.selectedSla === "Next day delivery"
					) {
						return {
						...
						if (activeSlot[0]) {
							;(deliveryWindow.startDateUtc = activeSlot[0].startDateUtc),
								...
								(deliveryWindow.price =
									typeof activeSlot[0].price == "string"
										? convertPriceToNumber(activeSlot[0].price)
										: activeSlot[0].price),
								(deliveryWindow.lisPrice =
									typeof activeSlot[0].price == "string"
										? convertPriceToNumber(activeSlot[0].price)
										: activeSlot[0].price),
							...
							if (slotData && slotData.slots.length > 0) {
								...
								;(deliveryWindow.startDateUtc = logInfo?.slas[0]?.availableDeliveryWindows?.filter(
									(del: any) => del.price == "0",
								)[0]?.startDateUtc),
									(deliveryWindow.endDateUtc = logInfo?.slas[0]?.availableDeliveryWindows?.filter(
										(del: any) => del.price == "0",
									)[0]?.endDateUtc)
								;(deliveryWindow.price = 0),
									(deliveryWindow.lisPrice = 0),
						...
						return {
							...
							selectedDeliveryInfo: logInfo.slas?.find(
								(sla: { id: any }) => sla.id == logInfo.selectedSla,
							)?.deliveryChannel,
							deliveryWindow: deliveryWindow?.startDateUtc
								? deliveryWindow
								: null,
						}
					}
				...
				await fetch(`/checkout-io/update-shipping`, {
					...
					.then(updateShippingResponse => {
						if (updateShippingResponse.ok) {
							...
							fetch(
								`/api/checkout/pub/orderForm/${orderFormId}/customData/tradeplace`,
								{
									method: "PUT",
									headers: {
										"Content-Type": "application/json",
								...
								.catch(err => {
									console.log("Error saving tradeplace custom data: ", err)
									history.push(routes.PAYMENT.route)
								})
						}
					})
	...
	useEffect(() => {
		if (standardDeliverySlot) {
			...
			logisticsInfo = logisticsInfo.map((info: any) => {
				if (
					info.selectedSla === "Standard free" ||
					info.selectedSla === "Standard charged" ||
					info.selectedSla === "Second day delivery" ||
					info.selectedSla === "Next day delivery"
				) {
	...
	return (
		<div className={styles.formWrapper}>
			...
			<form onSubmit={e => handleSubmit(e)}>
				<div className="mb5">
					<Input
						...
						errorMessage={
							submitError.postCode && !formData.postCode
								? intl.formatMessage(messages.inputError)
								: ""
						}
					/>
				</div>
				{isPostCodeValid && (
					<div className="mb5">
						<div
							...
							onFocus={() =>
								streetOptions.length && setIsAddressesListOpen(true)
							}
						>
							<div className="mb5">
								<div className={styles.formInputWrapper}>
									<Input
										...
										onChange={(e: any) => {
											handleChangeInputAddresses(e)
										}}
										placeholder={`${intl.formatMessage(
											messages.streetPlaceholder,
										)}`}
										errorMessage={
											submitError.street && !formData.street
												? intl.formatMessage(messages.inputError)
												: ""
										}
									/>
									{streetOptions.length ? (
										<div
											onClick={() =>
												streetOptions.length &&
												setIsAddressesListOpen(!isAddressesListOpen)
											}
											className={
												styles[
													isAddressesListOpen ? "caret-opened" : "caret-closed"
												]
											}
										>
							...
							<div
								className={styles.streetListWrapper}
								style={
									isAddressesListOpen
										? { display: "block" }
										: { display: "none" }
							...
							>
								{streetOptions.length
									? streetOptions.map((address: any, index: number) => {
											return (
												<div
													key={index}
													className={styles.singleStreet}
													onMouseDown={() => handleSingleAddress(address)}
												>
													{address.label}
												</div>
											)
						...
						<div className="mb5">
							<Input
								...
								placeholder={`${intl.formatMessage(
									messages.houseNumberPlaceholder,
								)}`}
						...
						<div className="mb5">
							<Input
								...
								errorMessage={(submitError.county && !formData.county) ? intl.formatMessage(messages.inputError) : (formData.county.includes("\x01") || formData.county.includes("\x02") || formData.county.includes("\x03") || formData.county.includes("\x04") || formData.county.includes("\x05") || formData.county.includes("\x06") || formData.county.includes("\x07") || formData.county.includes("\x08") || formData.county.includes("\x09") || formData.county.includes("\x0A") || formData.county.includes("\x0B") || formData.county.includes("\x0C") || formData.county.includes("\x0D") || formData.county.includes("\x0E") || formData.county.includes("\x0F") ? intl.formatMessage(messages.inputFormatError) : "")}
							/>
						</div>
						<div className="mb5">
							<p className={styles.stairsPreText}>
								{intl.formatMessage(messages.stairsPreText)}
							</p>
							<div className={styles.stairsInputWrapper}>
								<Toggle
									...
									label={
										<FormattedMessage id="checkout-hdx-delivery-slots-uk.stair-text" />
									}
								/>
							</div>
						</div>
						{slotData && (
							<Delivery
								prop={slotData}
								getSlots={loadDeliverySlots}
								activeSlot={activeSlot}
								setActiveSlot={setActiveSlot}
								shippingPrice={shippingPrice}
								setTradePlaceCustomData={setTradePlaceCustomData}
								showShipTogheter={showShipTogheter}
							/>
						)}
						<MessagesController
							itemsTypes={itemsTypes}
							hasSlots={slotData && slotData.slots.length > 0}
						/>

						{safeToLoadStandard && (
							<StandardDelivery
								orderForm={orderForm}
								onStandardDeliveryChanged={setStandardDeliverySlot}
							/>
						)}
						<div className={styles.goToPaymentWrap}>
							<Button type="submit">
								<FormattedMessage id="checkout-hdx-delivery-slots-uk.go-to-payment" />
							</Button>
						</div>
					</div>
				)}
				{!isPostCodeValid && formData.postCode.length > 0 && (
					<div className={styles.invalidPostalCode}>
			...
			{showRemoveInstallationModal && (
				<div
					className={styles.installationRemovedModalWrapper}
					onClick={() => {
						setShowRemoveInstallationModal(false)
						window.location.reload()
					}}
				>
					<div className={styles.installationRemovedModalContent}>
						<div
							className={styles.iconCloseInstallation}
							onClick={() => {
								setShowRemoveInstallationModal(false)
								window.location.reload()
							}}
						...
						<p>
							{intl.formatMessage(messages.installationRemove1)}
							<strong className="c-action-primary">{formData?.postCode}</strong>
							{intl.formatMessage(messages.installationRemove2)}
						</p>
					</div>
				</div>
			)}
...
const messages = defineMessages({
	...
	installationRemove2: {
		default:
			" is not covered by our Installation service. The installation fee will be removed.",
