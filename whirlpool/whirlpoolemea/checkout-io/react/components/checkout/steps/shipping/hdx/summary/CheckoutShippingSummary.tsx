...
const CheckoutShippingSummary: FC<CheckoutShippingSummaryProps> = ({
	...
	const shipTogether =
		orderForm?.customData?.customApps?.filter(app => app.id == "tradeplace")[0]
			?.fields.shipTogether == "true"
	const connectedGas =
		orderForm?.customData?.customApps?.filter(app => app.id == "tradeplace")[0]
			?.fields.connectedGas == "true"
	...
	if (connectedGas) itemsTypes = { ...itemsTypes, GAS: true }
		...
		fetch(`/app/hdx/delivery/slots/${orderForm?.orderFormId}`)
			...
			.then(data => {
				...
				refreshOrder().then(newOrder => {
					...
					let tradeplaceData = refreshedOrderForm?.customData?.customApps?.filter(
					...
					let scheduledSla = deliveries.filter(
						logInfo => logInfo.selectedSla === "Scheduled",
					)[0]
					...
					if (scheduledSla && !scheduledSla.slas[0].deliveryWindow) {
						...
						if (data?.slots && data.slots.length > 0) {
							...
							;(deliveryWindow.startDateUtc = scheduledSla.slas[0].availableDeliveryWindows?.filter(
								(del: any) => del.price == "0",
							)[0]?.startDateUtc),
								(deliveryWindow.endDateUtc = scheduledSla.slas[0].availableDeliveryWindows?.filter(
									(del: any) => del.price == "0",
								)[0]?.endDateUtc)
							;(deliveryWindow.price = 0),
								(deliveryWindow.lisPrice = 0),
						...
						let newScheduled = {
							...
							selectedDeliveryInfo: (scheduledSla.slas[0]
								.availableDeliveryWindows as any)?.filter(
								(del: any) => del.price == "0",
							)?.[0]?.deliveryChannel,
							deliveryWindow,
						}
						...
						let body = {
							...
							shipping: {
								...
								logisticsInfo: updatedLogistics,
							},
						}
				...
				let standardFreeSla = deliveries.filter(
					logInfo => logInfo.selectedSla === "Standard free",
				)[0]
				...
				if (
					...
					const updatedLogisticsInfo = deliveries.map(logInfo => {
						let isNextDay =
							logInfo.slas.filter(sla => sla.id === "Next day delivery")
								.length > 0
						let isSecondDay = logInfo.slas.filter(
							sla => sla.id === "Second day delivery",
						)
	...
	const convertDate = (date: string) => {
		let dateSplitted = new Date(date)
			.toUTCString()
			.toString()
			.split(" ")

		let day = daysAbb["1st"].includes(dateSplitted[1])
			? dateSplitted[1] + "st"
			: daysAbb["2nd"].includes(dateSplitted[1])
			? dateSplitted[1] + "nd"
			: dateSplitted[1] + "th"
		if (day.charAt(0) === "0") {
	...
	let itemsGroupedByDelivery: any = null
	...
	if (deliveries && orderForm) {
		...
		deliveries.map(delivery => {
			let currentDeliveryOverride = deliveriesOverrides.filter(
				(item: any) => item.deliveryId == delivery.selectedSla,
			)[0]
			if (!grouped[delivery.selectedSla]) {
				grouped[delivery.selectedSla] = {
					name:
						delivery.selectedSla === "Scheduled" && slots.length == 0
							? "We will contact you for Delivery and/or Installation"
							: connectedGas && delivery.selectedSla === "LeadTime"
							? "We will contact you for Delivery and/or Installation"
							: currentDeliveryOverride?.deliveryName
							? currentDeliveryOverride.deliveryName
							: delivery.selectedSla,
					position: currentDeliveryOverride?.position
						? currentDeliveryOverride.position
						: 100,

					products: [
						{
							name: orderForm.items.filter(pr => pr.id == delivery.itemId)[0]
								.name,
							leadtime: orderForm.items.filter(
								pr => pr.id == delivery.itemId,
							)[0].leadtime,
					...
					deliveryWindow:
						delivery.selectedSla === "Scheduled" && slots.length == 0
							? ""
							: delivery?.slas?.filter(
									sla => sla.id === delivery?.selectedSla,
							  )[0]?.deliveryWindow,
					shippingEstimate:
						delivery.selectedSla === "Scheduled" && slots.length == 0
							? ""
							: !currentDeliveryOverride?.hideBussinessDays
							? delivery.slas.filter(sla => sla.id === delivery.selectedSla)[0]
									.shippingEstimate
							: "",
				...
				grouped[delivery.selectedSla]?.products.push({
					...
					leadtime: orderForm.items.filter(pr => pr.id == delivery.itemId)[0]
						.leadtime,
					position: currentDeliveryOverride?.position
						? currentDeliveryOverride.position
						: 100,
		...
		if (shipTogether || (connectedGas && slots.length == 0)) {
			Object.keys(grouped).map(key => {
				if (
					key !== "shipTogether" &&
					deliveriesOverrides.filter((item: any) => item.deliveryId == key) &&
					deliveriesOverrides.filter((item: any) => item.deliveryId == key)[0]
						?.groupOnShipTogheter
				) {
					grouped["shipTogether"].products = grouped[
						"shipTogether"
					].products.concat([...grouped[key].products])
					delete grouped[key]
				}
			})
			grouped["shipTogether"].name =
				shipTogether || (connectedGas && slots.length == 0)
					? deliveriesOverrides.filter(
							(item: any) => item.deliveryId == "shipTogether",
					  )[0]
						? deliveriesOverrides.filter(
								(item: any) => item.deliveryId == "shipTogether",
						  )[0].deliveryName
					...
					: "Products that will be shipped togheter"
		}
		...
		<div className={style.shippingSummaryDelivery}>
			...
			{!dataLoading && (
				<div className={style.selectedDeliveries}>
					{Object.keys(itemsGroupedByDelivery).map(key => {
						return (
							<div
								className={style.selectedDeliveryWrapper}
								style={{ order: itemsGroupedByDelivery[key].position }}
							>
								<span
									className={[
										style.selectedDeliveryTitle,
										selectedDeliveryName === itemsGroupedByDelivery[key].name
											? style.selectedDeliveryTitleClicked
											: "",
									].join(" ")}
									onClick={() => {
										setSelectedDeliveryName(
											itemsGroupedByDelivery[key].name === selectedDeliveryName
												? ""
								...
								>
									...
									{itemsGroupedByDelivery[key].deliveryWindow && (
										<span>
											{" "}
											{convertDate(
												itemsGroupedByDelivery[key].deliveryWindow.endDateUtc,
											)}
										</span>
									)}
								</span>
								{selectedDeliveryName === itemsGroupedByDelivery[key].name && (
									<div className={style.selectedDeliveryItems}>
										<div className={style.selectedDeliveryItemsWrapper}>
											<div>
												{itemsGroupedByDelivery[key].products.map(
													(product: any) => {
														return (
															<div className={style.selectedDeliveryProduct}>
																{product.name}{" "}
																{product.leadtime && !product.hideLeadTime && (
																	<span
																		className={[
																			style.selectedDeliveryProductLeadTime,
																			"c-link",
																		].join(" ")}
																	>
																		({product.leadtime})
										...
										<div className={style.selectedDeliveryItemsWrapper}>
											<span
												className={style.selectedDeliveryItemsTitle}
											>{`${intl.formatMessage(messages.shippingPrice)}:`}</span>
											<div className={style.selectedDeliveryItemPrice}>
												{`${
													__RUNTIME__.culture.customCurrencySymbol
												} ${itemsGroupedByDelivery[key].price / 100}`}
											</div>
										</div>
									</div>
								)}
