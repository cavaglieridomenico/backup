...
const messages = defineMessages({
	...
	dirtyProfileTitle: {
		defaultMessage:
			"We’ve found an existing account, but ‘Phone number’ is missing",
	...
	dirtyProfileInfo2: {
		defaultMessage:
			"We need your contact number to keep you informed of your delivery progress on the day.",
		...
		<>
			...
			<div className={`${style.profileSummaryContainer} bg-checkout`}>
				<div className={`${style.profileSummaryEmail} flex mb4`}>
					...
					{!canEditData && (
						<div className={style.signInContainerSummary}>
							<div className={style.emailModalButtonSingIn}>{SummaryLogin}</div>
				...
				{!phone && profileHasDirtyData && (
					<div
						className={style.profileSummaryLabel + " c-red-warning"}
					>{`${intl.formatMessage(messages.phone)}: `}</div>
				)}
			...
			{profileHasDirtyData && (
				<div className={style.dirtyProfileContainer}>
					<div className={style.dirtyProfileMessageTitleWrap}>
						...
						<div
							className={
								style.dirtyProfileMessageTitle +
								" c-red-warning t-mini c-action-primary"
							}
						>
							{" "}
							{intl.formatMessage(messages.dirtyProfileTitle)}
						</div>
					</div>
					<div className={style.dirtyProfileMessageInfo}>
						<div className={style.dirtyProfileMessageInfoContainer}>
							<div className={style.dirtyProfileMessageInfoWrap}>
								<span className={style.dirtyProfileMessageInfoItem + " t-mini"}>
									{" "}
									{intl.formatMessage(messages.dirtyProfileInfo2) + " "}
								</span>
								<br />
								<span className={style.dirtyProfileMessageInfoItem + " t-mini"}>
									{" "}
								...
								{SummaryLogin}
								<span className={style.dirtyProfileMessageInfoItem + " t-mini"}>
									{" " + intl.formatMessage(messages.dirtyProfileInfo1)}
								</span>
							</div>
							<div
								className={
									style.dirtyProfileMessageInfoSignIn + " bg-action-primary"
								}
							>
								{SummaryLogin}
							</div>
						...
						<div
							className={[
								style.dirtyProfileMessageInfoWrap,
								style.dirtyProfileMessageInfoWrapBottom,
							].join(" ")}
						>
							<span className={style.dirtyProfileMessageInfoItem + " t-mini"}>
								{intl.formatMessage(messages.dirtyProfileInfo3)}
							</span>
							<span
								className={[
									style.signInChangeEmailSummary,
									style.signInChangeEmailSummaryDirty,
								].join(" ")}
								onClick={handleChangeEmail}
							>
							...
							<span className={style.dirtyProfileMessageInfoItem + " t-mini"}>
								{intl.formatMessage(messages.dirtyProfileInfo4)}
							</span>
						</div>
					</div>
				</div>
