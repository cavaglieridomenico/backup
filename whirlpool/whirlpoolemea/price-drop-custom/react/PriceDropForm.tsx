/**
 * @param {PriceDropFormProps}
 * @returns A form used for the Price Drop Alert subscription
 */
import React, { useState, useReducer, useEffect } from 'react'
import { useIntl } from 'react-intl'
import { usePriceDropForm, handleApi } from '../react/hooks/usePriceDropForm'
...
import { CheckboxProps, PriceDropFormProps } from './typings/global'
...
const PriceDropForm: StorefrontFunctionComponent<PriceDropFormProps> = ({
  ...
  errorRequiredField,
  errorInvalidMail,
  campaign = 'FORM_PDP_PRICEDROP',
  submitButtonText,
  successMessageNewsletter = 'Subscription to newsletter done succesfully',
  successMessagePriceDrop = 'Subscription to Price Drop Alert done succesfully',
  errorMessagePriceDrop = 'Subscription to Price Drop failed, retry',
  ...
  return (
    ...
    >
      {isFormLoading ? (
        ...
        <>
          {/* TITLE */}
          <h2 className={`${handles.container__title}`}>{title}</h2>
          {/* SUBTITLE TEXTS*/}
          ...
          >
            {statusNewsletter === 'LOADING' || loading ? (
              ...
              <Button
                ...
                disabled={statusPriceDrop === 'SUCCESS'}
              >
...
PriceDropForm.schema = {
  ...
  description:
    'Form that allow User to subscribe to Price Drop Alert (it will notify the User when price for the selected product will drop)',
  type: 'object',
  properties: {
    title: {
      title: 'Form Title',
      description: 'Set the text shown in the form title',
      default:"I want know when price drop",
      type: 'string',
    },
    subtitles: {
      type: 'array',
    ...
    successMessagePriceDrop: {
      title: 'Success Message Price Drop',
      default:"An e-mail will be send to you when the item price drop",
      description:
        'Message to display when the user successfully subscribe to Price Drop Alert',
      type: 'string',
    },
    successMessageNewsletter: {
    ...
    errorMessagePriceDrop: {
      title: 'Error Message to subscription Price Drop Alert',
      default:"Something wrong during subscription to Price Drop Alert",
      description:
        'Message to display when the the subscription to Price Drop Alert fail',
      type: 'string',
    },
    genericApiErrorMessage: {
