...
export async function sendPageEvents(e: PixelMessage) {
  ...
  switch (e.data.eventName) {
    ...
    // pageView & FEReady
    case 'vtex:pageView': {
      const path = window.location.pathname.split("/"). length >4 ? window.location.pathname.split("/").slice(0, 4).join('/') : window.location.pathname
      ...
      if(window.dataLayer.filter(e => e.event == 'pageView').length === 0){
        push({
          event: 'pageView',
          ...objToPush,
        })
