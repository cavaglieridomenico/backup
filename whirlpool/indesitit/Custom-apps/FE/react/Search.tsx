...
export default function Search({
  ...
  const handleData = (/* query: any, */ data: any, dataLayer: any, notFound: any) => {
    ...
    getUserSession().then((res: any) => {
        if(data.products && data.products.length && data.productSearch && data.productSearch.products.length){
          pushDataLayer(res, dataLayer, notFound);
        }
    });
  };
  const handleError = async () => {
      await fePages({ product: undefined }, undefined, dataLayer, "search", "Other");
      if(!dataLayer.find(el=>el.event==="feReady")){
        dataLayer.push({
          event: "errorPage",
