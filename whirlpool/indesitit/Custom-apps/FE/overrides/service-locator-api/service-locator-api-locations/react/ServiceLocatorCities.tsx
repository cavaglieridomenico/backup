...
const ServiceLocatorCities = (props: ServiceLocatorCitiesProps) => {
  ...
  useEffect(() => {
    ...
    const province = props.params.province.replace(/%20/g, " ");
    const region = props.params.region.replace(/%20/g, " ");
    setRegion(region)
    setProvince(province)
    let cities:any ={}
    fetch(
      `https://api.d2c.service-locator.wpsandwatch.com/v1/geo/cities?brand=IN&country=IT&locale=it_IT&province_id=${province}&region_id=${region}&country_id=Italia`,
      {
        headers: {
  ...
  return   <>
  <Helmet>
    <title> {province ? `Cerca un tecnico ${province} | Indesit Italia`: null } </title>
  </Helmet>
  <div className={classnames(styles.container)}>
  <h1 className={classnames(styles.title)}> Centro di Assistenza Autorizzato Indesit - {province}</h1>
