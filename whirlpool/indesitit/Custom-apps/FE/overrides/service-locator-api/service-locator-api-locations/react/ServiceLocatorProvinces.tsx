...
const ServiceLocatorProvinces = (props: ServiceLocatorProvincesProps) => {
  ...
  useEffect(() => {
    ...
    fetch(
      `https://api.d2c.service-locator.wpsandwatch.com/v1/geo/provinces?brand=IN&country=IT&locale=it_IT&region_id=${region}&country_id=Italia`,
      {
        headers: {
  ...
  return <>
  <Helmet>
    <title> {region ? `Cerca un tecnico ${region} | Indesit Italia`: null } </title>
  </Helmet>
  <div className={classnames(styles.container)}>
  <p className={classnames(styles.title)}> Centro di Assistenza Autorizzato Indesit - {region}</p>
