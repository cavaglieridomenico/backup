...
const FilterSidebar = ({
  ...
  const handleApply = () => {
    ...
    setFilterOperations([])
    /* console.log(filterOperations)    slide 69
    let counter
    ...
    filterOperations.map((toCheck) => toCheck.selected ? (counter > 0 ? counter-- : counter=0) : counter++)
    localStorage.setItem("filter", counter)
    console.log(counter) */
  ...
  const handleClearFilters = (key) => {
    ...
    //localStorage.removeItem("filter");    slide 69
        ...
        <FilterNavigatorContext.Provider value={context}>
          ...
          <div className={handles.buttons__container}>
           <span className={handles.resetButton__style} onClick={() => handleClearFilters()}>
            ...
            <ButtonPrimary text="APPLICARE" onClick={() => {handleApply()}} />
          </div>
      </FilterModal>
    </Fragment>
  )
