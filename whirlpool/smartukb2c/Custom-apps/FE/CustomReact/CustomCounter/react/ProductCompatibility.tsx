...
const ProductCompatibility: StorefrontFunctionComponent<Props> = ({ threshold, lowStockMessage, highStockMessage }) => {
  const valuesFromContext = useContext(ProductContext)
  if (!valuesFromContext || isEmpty(valuesFromContext)) {
    return null
  }
  const { product }: { product: product } = valuesFromContext
  const availabilityObject = path(['properties'], product) as string | undefined
  const availability =(availabilityObject?.filter( x => {  return x.name=="Status" || x.name=="Stato" || x.name=="Statut" || x.name=="status" || x.name=="stato" || x.name=="statut"  })[0]?.values[0])
  ...
  return<div> 
  {/* <div className={styles.availabilityFits}><FitApplianceCheck /><p id="fits-appliance">Check if this part fits your appliance </p></div>*/}
 <div className={styles.availabilityFits}><img id="fits-appliance-img" className={styles.availabilityFitsImg}src="/arquivos/checkProduct.png" crossorigin="anonymous" /><p id="fits-appliance">Check if this part fits your appliance </p></div>
  
  </div>
}

ProductCompatibility.defaultProps = {
  threshold: 3,
}
