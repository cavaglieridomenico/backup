...
const ProductAvailabilityWrapper: StorefrontFunctionComponent<Props> = ({ threshold, lowStockMessage, highStockMessage, product }) => {
  const valuesFromContext = useContext(ProductContext)
  if (!valuesFromContext || isEmpty(valuesFromContext)) {
    return null
  }

  const availability =  product.status ? product.status[0] : product.properties ? product.properties.filter((prop) => prop.originalName = "status")[0]?.values[0] : "out of stock";
