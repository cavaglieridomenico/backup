...
//import FindRightPart from "./utils/FindRightPart"
    ...
    <div className={styles.customAutocomplete__container}>
      {brand === 'indesit' ? (
        <div className={styles.sliderHeaderWrapper}>
          <p className={styles.customAutoComplete__header}>
            {' '}
            Welcome To Indesit Parts <br></br>Providing genuine spare parts and
            accessories direct from the manufacturer
          </p>
        ...
        <div className={styles.sliderHeaderWrapper}>
          <p className={styles.customAutoComplete__header}>
            {' '}
            Why To Hotpoint Parts ?<br></br>Original Parts,direct from the
            manufacturer. Safe and Interchangeability Guaranteed
          </p>
          {/* <h2>Providing genuine spare parts and accessories direct from the manufacturer</h2>
           <p className={styles.sliderHeader}>{slider.sliderHeader}</p> */}
        </div>
      )}
      ...
      <div className={styles.customAutocomplete}>
        <div className={styles.customAutocomplete__wrapper}>
          ...
          <input
            className={styles.customAutocomplete__input}
            placeholder={
              'Enter your model number, spare part code or keyword here'
            }
            onBlur={e => {
              setTimeout(() => {
                setResultsOpen(false)
          ...
          <a
            className={styles.customAutocomplete__search_btn}
            href={
              searchTerm !== null
                ? `${searchTerm}?_q=${searchTerm}&map=ft&showCode=` +
                  showCode +
        ...
        {resultsOpen && (
          <div className={styles.customAutocomplete__results}>
            {results.map((product, index) => {
              return (
                <a
                  className={styles.customAutocomplete__results_link}
                  href={`${encodeURI(
                    getPath(product['Link Text'] + '/p?showCode=' + showCode)
                  )}`}>
                  <div
                    className={
                      index === listCounter
                        ? styles.customAutocomplete__results_row__active
                        : styles.customAutocomplete__results_row
                    }>
                    <img
                      className={styles.customAutocomplete__results_row_image}
