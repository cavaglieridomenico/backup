...
export async function getProductCategory(ctx: Context, next: () => Promise<any>) {
    try {
        const {
            vtex: { route: { params: { id } } },  // Get "id" param in URL from context
            clients: { products }  // Get needed client from context
        ...
        // Implement request using product client
        const res = await products.getProductCategory(Number(id));
        // Return response
        ctx.body = res;
...
export async function getProductByRefId(ctx: Context, next: () => Promise<any>) {
    try {
        const {
            vtex: { route: { params: { id } } },  // Get "id" param in URL from context
            clients: { products }  // Get needed client from context
        ...
        // Implement request using product client
        const res = await products.getProductByRefId(String(id));
        // Return response
        ctx.body = res;
