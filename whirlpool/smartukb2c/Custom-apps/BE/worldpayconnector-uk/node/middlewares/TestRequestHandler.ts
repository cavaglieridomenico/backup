...
import { defaultCancelPaymentResponse, defaultCapturePaymentResponse, defaultCreatePaymentResponse, defaultRefundPaymentResponse, PaymentStatusVtex, vtexCredentials } from "../utils/constants";
        ...
        switch (request.card.number) {
            ...
            default: {
                ...
                setTimeout(() => {
                    ...
                    response.authorizationId = "testAuthorizationID"
                    response.nsu = "testNSU"
                    ExecuteHTTPRequest(request.callbackUrl, response, vtexCredentials[ctx.vtex.account])
                }, 5000);
            }
        }
        ctx.status = 200
