...
export default class VtexApiConfig extends JanusClient {
    ...
    public getProductDetails(productId: string): Promise<IOResponse<string>> {
    ...
    public async getLanguage(userId: string) {
        ...
        const language: string = (data[0].localeDefault !== null && data[0].localeDefault !== 'null') ? data[0].localeDefault.split('-')[0] : 'en';
        return language;
    }
}
