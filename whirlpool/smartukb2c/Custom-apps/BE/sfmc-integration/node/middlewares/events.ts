...
export async function backInStockEvent(ctx: any, next: () => Promise<any>) {
    ctx.vtex.logger = new CustomLogger(ctx);
    // Define setting and env
    const appSettings = await ctx.clients.apps.getAppSettings(`${process.env.VTEX_APP_ID}`);
    process.env.TEST = JSON.stringify(appSettings);
    ...
    // Call back in stock
    ctx.vtex.logger.info(logMessage("back in stock Event received"));
    console.log('**************** back in stock EVENT ****************')
    // cancellationOrder(ctx, next, true);


    ctx.body.customersToSend.forEach( (element:string|undefined) => {
        backInStock(ctx, next, true, element)
    });
