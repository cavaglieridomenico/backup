...
export async function buildEmailMessage(ctx: Context, type: string, brand: string, body: any) {
    ...
    return {
        ...
        Data: {
            ...
            deliveryType: body.shippingData.logisticsInfo[0].selectedSla,
...
export async function buildBackInStockMessage(ctx: Context, type: string, brand: string, body: any) {
    ...
    // Get SKU
    const sku = JSON.parse(JSON.stringify((await ctx.clients.vtexAPI.getProductSkuAlternative(body.skuId)).data));
    // Get main image URL
    const images = JSON.parse(JSON.stringify((await ctx.clients.vtexAPI.getImagesMain(body.skuId)).data));
...
export async function sendEmail(ctx: Context, type: string, brand: string, body: any, token: string) {
    ctx.vtex.logger = new CustomLogger(ctx);
    ...
    try {
        ...
        switch (type) {
            case "backInStock":
                message = await buildBackInStockMessage(ctx, type, brand, body);
                break;
            default:
                message = await buildEmailMessage(ctx, type, brand, body);
                break;
        }
    ...
    if (process.env.TEST) {
        if (type === 'confirmation') {
            if (brand === 'whirlpool') {
                ...
                key = JSON.parse(process.env.TEST).CONFIRMATION_APIEVENT_HOTPOINT;
            ...
            if (brand === 'whirlpool') {
                ...
                key = JSON.parse(process.env.TEST).CANCELLATION_APIEVENT_HOTPOINT;
            } else if (brand === 'indesit') {
                key = JSON.parse(process.env.TEST).CANCELLATION_APIEVENT_INDESIT;
            }
        } else if (type === 'backInStock') {
            if (brand === 'whirlpool') {
                key = JSON.parse(process.env.TEST).BACK_IN_STOCK_APIEVENT_WHIRLPOOL;
            } else if (brand === 'bauknecht') {
                key = JSON.parse(process.env.TEST).BACK_IN_STOCK_APIEVENT_BAUKNECHT;
            } else if (brand === 'hotpoint') {
                key = JSON.parse(process.env.TEST).BACK_IN_STOCK_APIEVENT_HOTPOINT;
            } else if (brand === 'indesit') {
                key = JSON.parse(process.env.TEST).BACK_IN_STOCK_APIEVENT_INDESIT;
            } else {
...
function getPaymentMethod(locale: string) {
    ...
    const paymentMethodEn = 'Credit Card'
    ...
    if (locale.includes('de')) {
        ...
        paymenthMetodTranslated = paymentMethodEn
    } else {
        paymenthMetodTranslated = paymentMethodDe
    }
