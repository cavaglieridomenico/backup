...
// import { useCartOrder } from "../../../providers/orderForm"
			...
			fetch('/checkout-io/checkItemStock', {
				...
				.then((data) => {
					...
					let cartItems = orderForm.items.map((item: any) => {
						let newItem = { ...item }
						if (data.messages && data.messages.arrayMessages && data.messages.arrayMessages.length > 0) {
					...
					{items?.map((item: any, index: number) => {
						return (
							<ItemContextWrapper
								...
								{...props}
							>
								<div className={[style.itemWrap, item.toBeRemoved ? "bg-checkout " +  style.itemWrapToBeRemoved :  ""].join(" ")}>
									{item.toBeRemoved && (
										<div className={["bg-action-primary c-on-action-primary", style.itemToBeRemovedText].join(" ")}>This product is no longer available and will be removed from your order.</div>
									)}
									{children}
								</div>
							</ItemContextWrapper>
