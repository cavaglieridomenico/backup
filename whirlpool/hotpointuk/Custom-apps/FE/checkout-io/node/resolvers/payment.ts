...
export const paymentMutations = {
  ...
  mutationCreatePaymentFromExistingCart: async (
    ...
    try {
      ...
      // 1. Place Order
      let placeOrderResponse = await paymentClient.placeOrderFromExistingCart(params, ctx.vtex.logger, CheckoutOrderFormOwnershipCookie);
      ...
      if (merchantTransactions?.length > 0) {
        ...
        // }${gatewayCallbackTemplatePath}`
        const callbackUrl = `${`https://${ctx.vtex.host}`
          }${gatewayCallbackTemplatePath}`
        ...
        const paymentsResponse = await paymentClient.processOrderTransaction(
          ...
          paymentClient.calculatePaymentsFromPlaceOrder(placeOrderResponse),
          callbackUrl,
          orderGroup,
        ...
        if (paymentsResponse.status === 201 || paymentsResponse.status === 200) {
          ...
          //   'Success'
          // )}`
          redirectUrl = `${`https://${ctx.vtex.host}`
            }${gatewayCallbackTemplatePath.replace(
              '{messageCode}',
              'Success'
            )}`
  ...
  mutationAddPaymentData: async (
    ...
    try {
      ...
      // 1. Place Order
      let addPaymentDataResponse = await paymentClient.addPaymentData(orderFormId, body as [AddPaymentDataInput], CheckoutOrderFormOwnershipCookie)
