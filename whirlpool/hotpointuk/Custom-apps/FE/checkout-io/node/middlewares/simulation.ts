...
export async function simulation(ctx: Context, next: () => Promise<any>) {
  ...
  try {
    const currentOrderForm = await checkoutClient.orderForm(orderFormId, ctx.state.CheckoutOrderFormOwnershipCookie)
    ...
    const response = await checkoutClient.simulation(_simulation, ctx.state.CheckoutOrderFormOwnershipCookie)
