...
const DiscontinuedProducts: StorefrontFunctionComponent<PropsDiscontinuedProduct> = ({
    ...
    onCompleted: (data: any) => {
      ...
      setRender(
        <ProductWrapperDiscontinued
          ...
          query={{skuId:data?.product?.items[0]?.itemId, idsku: data?.product?.items[0]?.itemId}}
          productId={id}
        >
          {children}
        </ProductWrapperDiscontinued>
  ...
  // --- OLD WAY --- It caused error "Rendered more hooks than during the previous render" sometimes
  // useEffect(() =>{
