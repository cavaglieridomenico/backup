...
import { DeliveryProps, Slot, formatDate, shippingPolicies } from "./utils/utilsForDelivery";
...
const Delivery: FC<DeliveryProps> = ({
  ...
  const [shipAllProductTogether, setShipAllProductTogether] = useState<any>(false)


  useEffect(() => {
    setShipAllProductTogether(tradeplaceCustomData ? tradeplaceCustomData.shipTogether == "true" : false)
  ...
  //const productsDeliveryValues: Array<number | undefined> = orderForm?.items?.map((item: { offerings: Offering[] }) => item.offerings?.find((offering) => offering.name === "Delivery")?.price).filter((item: any) => !!item)
  ...
  const currencySymbol: string = orderForm?.storePreferencesData?.currencySymbol */
  const intl = useIntl()
  ...
  useEffect(() => {
    if (prop.slots && prop.slots.length > 0) {
      ...
      prop.slots.map((slot: Slot) => {
        let { weekday, month, dayNumber, startHour, endHour } = formatDate(slot.startDateUtc, slot.endDateUtc)
        data.push({ weekday, month, dayNumber, startDateUtc: slot.startDateUtc, endDateUtc: slot.endDateUtc, price: (shippingPrice === 0 ? "FREE" : "£" + shippingPrice / 100), active: false, startHour, endHour })
      })
      setSlots(data)
    }
  }, [])
  ...
  const setDelivery = (slot: any) => {
    ...
    data.forEach((singleSlot: any) => {
      if (slot.startDateUtc === singleSlot.startDateUtc)
        ...
        singleSlot.active = false
    })
    setActiveSlot(data.filter((singleSlot: any) => { return slot.startDateUtc === singleSlot.startDateUtc }))
    setSlots(data)
  }
  ...
  useEffect(() => {
    ...
    if (itemsTypes.MDA) {
      let scheduledDeliveryAvailable = logisticInfos?.some(
        (logInfo: any) => logInfo?.slas?.some(
          (sla: any) => sla?.id == "Scheduled" && sla?.availableDeliveryWindows?.length > 0 || false)
          || false)
        || false;
      if (!scheduledDeliveryAvailable && tradeplaceCustomData && tradeplaceCustomData.tpError !== "true") {
        newTPCustomData.tpError = "true";
      }
      else if (scheduledDeliveryAvailable && tradeplaceCustomData && tradeplaceCustomData.tpError !== "false") {
  ...
  return (
    <>
      ...
      {itemsTypes.MDA && itemsTypes.OOS && !itemsTypes.GAS && (
        <>
          <p className={styles.shipAllTogheterText}><FormattedMessage id="checkout-hdx-delivery-slots-uk.toggle-ship-products-all-together" /></p>
          <div className={styles.shipAllTogheterToggle}>
            <Toggle
              checked={shipAllProductTogether}
              id="stairs"
              onChange={(e: any) => { handleShipAllTogether(e) }}
              label={intl.formatMessage(messages.shipAllTogether)}
            />
          </div>
        </>)}
      {prop && prop.slots?.length > 0 && !shipAllProductTogether && <div className={styles.customdate__wrapper}>
        <p className={[styles.shiptext, styles.deliveryText].join(" ")}><FormattedMessage id="checkout-hdx-delivery-slots-uk.preferred-delivery-date" /></p>

        <div className={styles.customdate__table}>
          {slots && slots.length > 0 && slots.slice(0, 6).map((slot: any, index: number) => {
            return (
              <div key={index} className={`${styles.customdate__table_item} ${slot.startDateUtc === activeSlot[0]?.startDateUtc ? styles.customdate__table_item_active : ""}`} onClick={() => setDelivery(slot)}>
                <b>
                  {slot.weekday}
                </b>
                <b> {slot.dayNumber.replace(/^0+/, '')} {slot.month}</b>
                {/*                 <span>
                  {productsDeliveryTotal ? `(${(currencySymbol + (productsDeliveryTotal / 100).toFixed(2).replace(",", "."))})` : ""}
                </span> */}
              </div>
            )
          })}
        </div>
        <p className={styles.selectPreText}><FormattedMessage id="checkout-hdx-delivery-slots-uk.more-dates" /></p>
        <select defaultValue={'DEFAULT'} onChange={(e: any) => setDelivery(slots[e.target.value])}>
          <option value="DEFAULT" disabled>{intl.formatMessage(messages.moreDates)}</option>
          {slots && slots.length > 0 &&
            slots.map((slot: any, index: number) => {
              return <option key={index} value={index} selected={slot.active}>
                {slot.weekday} {slot.dayNumber.replace(/^0+/, '')} {slot.month} {/* {productsDeliveryTotal ? `(${(currencySymbol + (productsDeliveryTotal / 100).toFixed(2).replace(",", "."))})` : ""} */}
              </option>
            })
          }
        </select>
        <p className={styles.selectPreText}><FormattedMessage id="checkout-hdx-delivery-slots-uk.select-time" /></p>
        <select defaultValue={'DEFAULT'} onChange={() => { }}>
          <option value="DEFAULT" disabled>{intl.formatMessage(messages.moreTime)}</option>
          {activeSlot && activeSlot.length > 0 &&
      ...
      {prop.slots?.length == 0 && (itemsTypes.MDA || itemsTypes.SDA) && (!itemsTypes.GAS || itemsTypes.OOS) && (
        <p className={[styles.shiptext, styles.deliveryText].join(" ")}><FormattedMessage id="checkout-hdx-delivery-slots-uk.preferred-delivery-date" /></p>
...
const messages = defineMessages({
  moreDates: {
    ...
    id: "checkout-hdx-delivery-slots-uk.more-dates",
  },
  moreTime: {
