...
const YourekoBadge: StorefrontFunctionComponent = () => {
	function getYoureko() {
		...
		script.async = true
		script.src =
			'https://static.youreko.com/js/partners/uk/hotpoint/youreko.energy-review.hotpoint.all.min.js'
	...
	const prodId = productInfo?.product?.productReference?.split('-')[0]
	const prodName = productInfo?.product?.productName
	const productLink = productInfo?.product?.linkText?.split('-').slice(-1)[0]

	function getYourekoCallback(category: string) {
		// eslint-disable-next-line no-console

		if (
			document?.getElementById('button-youreko-callback-' + productLink) ===
			null
		) {
			const script = document.createElement('script')
