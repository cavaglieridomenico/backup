...
const Dropdown: StorefrontFunctionComponent<DropdownProps> = ({
  ...
  const contactArea = ["Spare Parts Landing", "Product Page", "Models Page", "Product Listing Page"];
  ...
  function selectedContactArea (){
    ...
    let categorySelected = contactArea[1];
    if ("spare-parts" === category){
      categorySelected = contactArea[0];
    }else if (category.indexOf("spare-parts") != -1 ) {
      categorySelected = contactArea[3];
    }
    if (category.indexOf("spare-parts-model-number-lookup") != -1 ){
      categorySelected = contactArea[2];
    }
