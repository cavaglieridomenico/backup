...
const SearchHomePage: StorefrontFunctionComponent<SearchProps> = ({
  ...
  const buildUrl = () => {
    ...
    if (selectedIndustrial) {
      url = selectedIndustrial["modelNumber"] ? `/spare-parts/bom/${normalizeModelNumber(selectedIndustrial["modelNumber"]).toLowerCase()}-${selectedIndustrial["industrialCode"].toLowerCase()}` : `/spare-parts/bom/${selectedIndustrial["industrialCode"].toLowerCase()}`;
      url = `${url}${selectedItem && selectedItem !== "All" ? "?category=" + selectedItem : ""}`
    }
          ...
          <div className={styles.custom_search}>
            ...
            {showResults && inputValue && results && results.length > 0 && (
              <div className={styles.custom_search_results}>
                <div className={styles.custom_search_results_count}>{results.length} results</div>
            ...
            {selectedIndustrial &&
              <div className={[styles.dropdown, isOpen ? styles.dropdown_open : "", selectedItem ? styles.dropdown_selected : "", categories.length === 0 ? styles.dropdown_disabled : ""].join(" ")}>
                ...
                <label className={styles.custom_search_input_label}>Category / Part type</label>
