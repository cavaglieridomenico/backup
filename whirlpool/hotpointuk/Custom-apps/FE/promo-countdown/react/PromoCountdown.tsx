...
const PromoCountdown: StorefrontFunctionComponent<PromoCountdownProps> = ({ }) => {
  ...
  const checkIsDiscontinued = (product: any) => {
    const isDiscontinued = product?.specificationGroups?.find((el: any) => el.name === "allSpecifications")?.specifications?.find((el:any) => el.name === "isDiscontinued")?.values?.[0];
  ...
  return (
    <>
      {isDiscontinued === "false" && startDate - now <= 0 && endDate - now > 0 && (
        <div className={`${handles.countdownContainer} w-100`}>
          <p className={`${handles.countdownText} c-action-primary t-body b`}>
            {intl.formatMessage({id: "store/promo-countdown.expires"})}
            {daysLeft}{intl.formatMessage({id: "store/promo-countdown.days"})}
            {hoursLeft}{intl.formatMessage({id: "store/promo-countdown.hours"})}
            {minutesLeft}{intl.formatMessage({id: "store/promo-countdown.minutes"})}
            {secondsLeft}{intl.formatMessage({id: "store/promo-countdown.seconds"})}
          </p>
