...
export const BreadcrumbPdpCat: StorefrontFunctionComponent<BreadcrumbPdpCatProps> = () => {
  ...
  const isDiscontinued =
		productContext?.product?.specificationGroups
			...
			?.specifications?.find((el: any) => el.name === 'isDiscontinued')
			?.values?.[0] || false

  const prePlp = !isOutOfStock && productContext?.product?.categories.length > 2 && !isDiscontinued
    ? productContext?.product?.categories?.[0].split(/[//]/)?.[2]
    : isDiscontinued || isOutOfStock
      ? productContext?.product?.categories[1].split(/[//]/)[2]
      : productContext?.product?.categories?.[0].split(/[//]/)?.[1]
  const plpCategoryLink = !isOutOfStock ? productContext?.product?.categories?.[0]?.replace(" ","-").slice(0,-1) : productContext?.product?.categories?.length > 2 ? productContext?.product?.categories?.[2].replace(" ","-").slice(0,-1) : ""
  const productCategory = productContext?.product?.categories.length > 2 ? productContext?.product?.categories?.[0].split(/[//]/)[3] || productContext?.product?.categories?.[2].split(/[//]/)[3] : productContext?.product?.categories?.[0].split(/[//]/)[2] || productContext?.product?.categories?.[1].split(/[//]/)[2]
  const productComCode = productContext?.product?.properties?.filter((e: any) => e.name == "CommercialCode_field")[0]?.values?.[0]
  const prePlpLink = "/" + productCategory?.replace(" ","-");
  ...
  const plpCategoryLink2 = productCategory2 !== productCategory ? productContext?.product?.categories?.[3].replace(" ","-").slice(0,-1) : plpCategoryLink;
    ...
    (<>
      <Helmet>
        ...
        <div>
          ...
          <a className={style.catLink} href={prePlpLink}>
            {prePlp}
          </a>
          <img className={style.arrowImg} src={"https://hotpointuk.vtexassets.com/assets/vtex/assets-builder/hotpointuk.hotpoint-theme/1.0.32/arrowIconBread___a656b762fc2fb83036cf9d96da6f14a0.svg"}/>
