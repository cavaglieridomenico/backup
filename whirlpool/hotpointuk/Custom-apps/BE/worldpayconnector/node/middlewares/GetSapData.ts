...
import { Acquirer, authHeaders, AUTH_FLAG, CC_REACT, CC_STAT_EX, DATAORIGIN, orderSuffix } from "../utils/constants"
...
export async function GetSapData(ctx: Context, next: () => Promise<any>) {
  ...
  try {
    payment = await GetPayment(ctx, orderid.includes(orderSuffix) ? orderid : orderid + orderSuffix, 2500, 2)
  } catch (err) {
