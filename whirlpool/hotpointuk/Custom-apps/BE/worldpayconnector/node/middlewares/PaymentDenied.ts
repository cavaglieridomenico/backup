...
export async function PaymentDenied(ctx: Context, next: () => Promise<any>) {
  ...
  logger.info("[Payment Denied] order:" + SanitizeString(orderid))
  ...
  try {
    ...
    logger.error("[Payment Denied] order:" + SanitizeString(orderid) + " - Error retrieving payment")
    logger.debug(err)
  }

  if (existingPayment) {
    if (authToken != existingPayment.denytoken) {
      logger.error("[Payment Denied] order:" + existingPayment.orderid + " - Invalid token")
      throw new AuthenticationError("Invalid token")
    }
    logger.info("[Payment Denied] order:" + existingPayment.orderid + " - Updating Master data")
    ...
    await ctx.clients.paymentProvider.callback(existingPayment.transactionid, existingPayment.id, auhtCallback).catch(
      (err) => {
        logger.error("[Payment Denied] order:" + existingPayment.orderid + " - Callback error")
        logger.debug(err)
      }
    )
  }
...
const SanitizeString = (string: string) => string.replace(/[^A-Za-z0-9-]/g, "")
