...
export async function UpdateOutSubscriptions({ state, vtex: { logger }, clients: { masterdata } }: Context, next: () => Promise<any>) {
  ...
  const requestQueue = new RequestQueue(() => {
    logger.info(`[UpdateOutSubscriptions] - Update completed with ${errors} errors`)
  }, 50)
  ...
  toUpdate.forEach(sub => requestQueue.PushRequest({
    ...
    callback: (_, err) => {
      if (err) {
        errors++
        logger.error(`[UpdateOutSubscriptions] - Error updating subscription: ${JSON.stringify(sub)}`)
      }
    }
  }))
