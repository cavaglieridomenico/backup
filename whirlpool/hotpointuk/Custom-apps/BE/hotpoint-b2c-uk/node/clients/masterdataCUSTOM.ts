...
export default class MD extends ExternalClient {
    ...
    public async getSparePartId(whereCondition: any, page: number, pageSize: number,ctx:any) {
        ...
        try {
            let response = await fetch(`https://${ACCOUNT}.vtexcommercestable.com.br/api/dataentities/${entity}/search?_where=${whereCondition}&_fields=sparePartId,familyGroup&_sort=${sortingValue}`, {
            headers: {
