...
export async function sendEnhancedEcommerceEvents(e: PixelMessage) {
  switch (e.data.eventName) {
    ...
    case "vtex:addToCart": {
      ...
      let dim5Value = costructionType(findDimension(data, "type de construction"));
      let dim4Value =
        (await findSellable(data)) == "true"
          ? "Sellable Online"
          : "Not Sellable Online";
    ...
    case "vtex:productImpression": {
      ...
      Promise.all(parsedImpressions).then((values:[unknown, unknown, unknown, unknown, unknown, unknown, unknown, unknown, unknown, unknown]) => {
...
function findVariantImpression(data: any) {
  const result = data.properties.filter((el:any) => el.name === "Couleur");
      ...
      Promise.all([
        ...
        var obj = {
          ...
          dimension4: costructionType(
            findDimension(values[0], "type de construction")
          ),
          dimension5:
