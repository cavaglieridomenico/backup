...
function uploadProductSpecifications() {
    ...
    for(var c of Object.keys(productXCategory)){
        ...
        productXCategory[c].forEach(p => {
                  ...
                  case "Endeca":
                      if(r1.description!="" && r1.value!=""){
                        if(r1.description.toLowerCase()=="funzioni speciali"){
                          productSpecification.FieldId=specificationList[c][r1.groupname][r1.description+"_"+specialFunctionId]?specificationList[c][r1.groupname][r1.description+"_"+specialFunctionId]:findId(c.r1.groupname,r1.description+"_"+specialFunctionId);
                          productSpecification.Text=r1.value;
                          if(valuesArrays[c][p][r1.groupname]==undefined){
                            valuesArrays[c][p][r1.groupname]={};
                          }
                          params.push(r1.groupname);
                          params.push(r1.description+"_"+specialFunctionId);
                          API.sendRequest("/api/catalog/pvt/product/"+productIDmapping[r1.partnumber]+"/specification", "POST", productSpecification, (response,params2) => {
                            let jsonData = JSON.parse(response);
                            valuesArrays[params2[0]][params2[1]][params2[2]][params2[3]]=jsonData.Id;
                          }, params);
          ...
          let productSpecification0 = [
            {
                Value: [
                    1
                ],
                Id: specificationList[c]["Other"]["fewPiecesThreshold"]
            }
          ];
          API.sendRequest("/api/catalog_system/pvt/products/"+productIDmapping[p]+"/specification", "POST", productSpecification0, null, null);

          let productSpecification1 = [
            {
                Value: [
                    1
                ],
                Id: specificationList[c]["Other"]["minimumQuantityThreshold"]
            }
          ];
          API.sendRequest("/api/catalog_system/pvt/products/"+productIDmapping[p]+"/specification", "POST", productSpecification1, null, null);
    ...
    API.stop(/*()=>{fs.writeFileSync("./new/productSkuSpecifications.json", JSON.stringify(valuesArrays, null, 2))}*/);
}
