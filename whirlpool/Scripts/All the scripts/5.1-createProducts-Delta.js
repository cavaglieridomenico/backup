...
function createProducts(){
    ...
    catalogEntry.root.record.filter(r0 => r0.type=="ProductBean" && !r0.partnumber.includes("SERVICE"))?.forEach(p0 => {
        if(productIdMapping[p0.partnumber]==undefined){
            ...
            API.sendRequest('/api/catalog/pvt/product/', 'POST', product, (responsedata, info) => {
                ...
                if(productIdMapping[info[0]]==undefined){
                    productIdMapping[info[0]]=jsonData.Id;
                }
                delta[info[0]]=jsonData.Id;
                let cat = info[1];
                if(cat.length > 1){
    ...
    API.stop(()=>{
        fs.writeFileSync("./BODL2JsonMapping/BODLProductIdToVtexProductId.json", JSON.stringify(productIdMapping, null, 2));
        fs.writeFileSync("./new/productDelta.json", JSON.stringify(delta, null, 2));
    });
}
