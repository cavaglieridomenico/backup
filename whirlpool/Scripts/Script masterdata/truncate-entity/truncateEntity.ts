...
const APP_KEY = "vtexappkey-hotpointukqa-VZOBMM"
const APP_TOKEN = "RQDLMPKEFKDAFPOXVFFTEFGCKIVJWYTKSSWGDXLHJUONQTHUWFEMDVCMKBBDTAHHQTINSIUXTXJKQSLQBBNQUHGXQKWBWINJMHXWLTBNLGSGLRVRPKCSNRNJNKWHLSBL"
...
async function main() {
  ...
  for (let i = 0; i < masterdataFullContent.length; i += chunkSize) {
    ...
    const promiseResponses: Promise<boolean>[] = chunk.map((record) => deleteRecord(record, ENTITY))
...
function deleteRecord(record: any, entity: string): Promise<boolean> {
  return new Promise(async (resolve, reject) => {
    fetch(`https://${ACCOUNT}.myvtex.com/api/dataentities/${entity}/documents/${record.id}`, optionsDelete)
      .then((mdResponse) => {
        if (mdResponse.ok) {
